...
#define PCM_PULSE_OUTPUT_LOG_PEROID            (48000*5)
...
static int pulse_start(snd_pcm_ioplug_t * io)
{
	...
	assert(pcm);
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_start device=%s.name=%s\n",pcm->device,pcm->io.name);
#endif
	...
	pcm->underrun = 0;
	/* no wait for state at start sequence*/
	#if 0
...
static int pulse_stop(snd_pcm_ioplug_t * io)
{
	...
	assert(pcm);
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_stop device=%s.name=%s.\n",pcm->device,pcm->io.name);
#endif
	if (!pcm->p || !pcm->p->mainloop)
		return -EBADFD;
				     ...
				     snd_pcm_uframes_t size)
{
	...
	{
#ifdef HAVE_HLOG
        HLOGE(TAG_HARMAN_PULSEAUDIO,"pulse_write device=%s.name=%s. NULL point.\n",pcm->device,pcm->io.name);
#endif
		return -EBADFD;
	}
	pa_threaded_mainloop_lock(pcm->p->mainloop);
	pcm->write_req += size;
	if(pcm->write_req >= PCM_PULSE_OUTPUT_LOG_PEROID)
	{
		pcm->write_req = 0;
#ifdef HAVE_HLOG
		HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_write device=%s,name=%s.written=%lld.\n",pcm->device,pcm->io.name,pcm->written);
#endif
	}
	ret = check_stream(pcm);
	if (ret < 0)
...
static void stream_underrun_cb(pa_stream * p, void *userdata)
{
	...
	if (do_underrun_detect(pcm, p))
	{
		...
		perror("underrun");
#ifdef HAVE_HLOG
		HLOGE(TAG_HARMAN_PULSEAUDIO,"ioplug underrun\n");
#endif
	}
}
...
static int pulse_prepare(snd_pcm_ioplug_t * io)
{
	...
	assert(pcm);
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_prepare device=%s.name=%s.\n",pcm->device,pcm->io.name);
#endif
	if (!pcm->p || !pcm->p->mainloop)
		return -EBADFD;
...
#else
	for (c = pcm->ss.channels; c > 0; c--)
		if (pa_channel_map_init_auto(&map, c, PA_CHANNEL_MAP_AUX))
			break;
	...
	if(io->name[0] =='_'){
		strncpy(stream_name,&io->name[1],strnlen(io->name,63)-1);
	}else{
		strncpy(stream_name,io->name,strnlen(io->name,63));
	}
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_prepare stream_name=%s,io->name=%s,io->name length=%d.\n",stream_name,io->name,strnlen(io->name,63));
#endif
	pcm->stream =
		pa_stream_new(pcm->p->context, stream_name, &pcm->ss, &map);
#endif
	...
	if (io->stream == SND_PCM_STREAM_PLAYBACK) {
		...
		r = pa_stream_connect_record(pcm->stream, pcm->device,
					     ...
					     | (pcm->no_mute ? PA_STREAM_NO_MUTE_ALWAYS : 0)
#ifdef PA_STREAM_EARLY_REQUESTS
					     | PA_STREAM_EARLY_REQUESTS
#endif
			);
	...
	if (r < 0) {
#ifdef HAVE_HLOG
		HLOGE(TAG_HARMAN_PULSEAUDIO,"PulseAudio: Unable to create stream: %s\n", pa_strerror(pa_context_errno(pcm->p->context)));
#endif
		pa_stream_unref(pcm->stream);
		pcm->stream = NULL;
		r = -EIO;
			   ...
			   snd_pcm_hw_params_t * params)
{
	...
	default:
#ifdef HAVE_HLOG
		HLOGE(TAG_HARMAN_PULSEAUDIO,"PulseAudio: Unsupported format %s\n",
	...
	pa_threaded_mainloop_unlock(pcm->p->mainloop);
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_hw_params device=%s.name=%s.channels=%d,rate=%d,buf_size=%d,peroid_size=%d.\n",pcm->device,pcm->io.name,io->channels,io->rate,io->buffer_size,io->period_size);
#endif
	return err;
}
...
static int pulse_close(snd_pcm_ioplug_t * io)
{
	...
	assert(pcm);
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_close device=%s.name=%s.\n",pcm->device,pcm->io.name);
#endif
	if (pcm->p && pcm->p->mainloop) {
...
static int pulse_pause(snd_pcm_ioplug_t * io, int enable)
{
	...
	assert (pcm);
#ifdef HAVE_HLOG
	HLOGI(TAG_HARMAN_PULSEAUDIO,"pulse_pause device=%s.name=%s.\n",pcm->device,pcm->io.name);
#endif
	if (!pcm->p || !pcm->p->mainloop)
		return -EBADFD;
...
SND_PCM_PLUGIN_DEFINE_FUNC(pulse)
{
	...
	snd_config_for_each(i, next, conf) {
...
#ifdef HAVE_HLOG
				HLOGE(TAG_HARMAN_PULSEAUDIO,"Invalid type for %s", id);
#endif
				return -EINVAL;
			}
			continue;
...
#ifdef HAVE_HLOG
				HLOGE(TAG_HARMAN_PULSEAUDIO,"Invalid value for %s", id);
#endif
				return -EINVAL;
			}
			handle_underrun = err;
		...
		if (strcmp(id, "rtprio") == 0) {
			if (snd_config_get_integer(n, &rtprio) < 0) {
#ifdef HAVE_HLOG
				HLOGE(TAG_HARMAN_PULSEAUDIO,"Invalid value for %s", id);
#endif
				return -EINVAL;
...
#ifdef HAVE_HLOG
		HLOGE(TAG_HARMAN_PULSEAUDIO,"Unknown field %s", id);
#endif
		return -EINVAL;
	}
