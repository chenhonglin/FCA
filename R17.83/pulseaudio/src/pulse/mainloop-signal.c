...
static int pa_backtrace_flag = 0;

static void signal_print_backtrace(int sig)
{
    switch(sig)
    {
    case SIGABRT:
    ...
    case SIGSYS:
#endif
        pa_log("signal_print_backtrace,sig=%d.",sig);
        pashow_backtrace();
        pa_backtrace_flag =1;
        break;
    default:
...
static void signal_handler(int sig) {
    ...
    saved_errno = errno;
#ifdef HAVE_PBACKTRACE
    if(pa_backtrace_flag ==0)
    {
        signal_print_backtrace(sig);
    }
#endif
