...
#ifdef HAVE_PBACKTRACE
void pashow_backtrace(void)
{
    void *pstack_handle;
    void (*pcallstack_call)(void);

    pstack_handle = dlopen(PBACK_TRACE_LIBPATH, RTLD_NOW);
    if(pstack_handle == NULL){
        pa_log("dlopen %s failed",PBACK_TRACE_LIBPATH);
        return ;
    }
    pcallstack_call = dlsym(pstack_handle, "pbacktrace");
    if(NULL == pcallstack_call){
        pa_log("dlsym %s failed.(%s)", pstack_handle, dlerror());
        return ;
    }
    pcallstack_call();
}
    ...
    for (t = text; t; t = n) {
        ...
        switch (_target) {
            ...
            case PA_LOG_HLOG: {
                ...
                if(level == PA_LOG_ERROR)
                {
                    HLOGE(TAG_HARMAN_PULSEAUDIO, "%s%s%s%s%s%s%s\n", timestamp, location, prefix, t, grey, pa_strempty(bt), suffix);
                }else if(level == PA_LOG_WARN)
                {
                    HLOGW(TAG_HARMAN_PULSEAUDIO, "%s%s%s%s%s%s%s\n", timestamp, location, prefix, t, grey, pa_strempty(bt), suffix);
                }
                else if(level == PA_LOG_NOTICE)
                {
                    HLOGV(TAG_HARMAN_PULSEAUDIO, "%s%s%s%s%s%s%s\n", timestamp, location, prefix, t, grey, pa_strempty(bt), suffix);
                }
                else if(level == PA_LOG_INFO)
                {
                    HLOGI(TAG_HARMAN_PULSEAUDIO, "%s%s%s%s%s%s%s\n", timestamp, location, prefix, t, grey, pa_strempty(bt), suffix);
                }
                else if(level == PA_LOG_DEBUG)
                {
                    HLOGD(TAG_HARMAN_PULSEAUDIO, "%s%s%s%s%s%s%s\n", timestamp, location, prefix, t, grey, pa_strempty(bt), suffix);
                }
                else
                {
                    HLOGD(TAG_HARMAN_PULSEAUDIO, "%s%s%s%s%s%s%s\n", timestamp, location, prefix, t, grey, pa_strempty(bt), suffix);
...
#ifdef HAVE_HLOG
    else if (pa_streq(string, "hlog"))
        t = pa_log_target_new(PA_LOG_HLOG, NULL);
#endif
    else
