...
static int pa_cli_command_unload_alsa_sink_module(pa_core *c, pa_tokenizer *t, pa_strbuf *buf, bool *fail);
static int pa_cli_command_unload_alsa_source_module(pa_core *c, pa_tokenizer *t, pa_strbuf *buf, bool *fail);
static int pa_cli_command_unload_alsa_module(pa_core *c, pa_tokenizer *t, pa_strbuf *buf, bool *fail, const char *module_name );
...
static const struct command commands[] = {
    ...
    { "unload-module-alsa-sink", pa_cli_command_unload_alsa_sink_module, "Unload a module-alsa-sink module (args: sink_name=sink name)", 2},
    { "unload-module-alsa-source", pa_cli_command_unload_alsa_source_module, "Unload a module-alsa-source module (args: source_name=source name)", 2},
...
static int pa_cli_command_unload_alsa_sink_module(pa_core *c, pa_tokenizer *t, pa_strbuf *buf, bool *fail) {
    return pa_cli_command_unload_alsa_module(c, t, buf, fail, "module-alsa-sink");
}

static int pa_cli_command_unload_alsa_source_module(pa_core *c, pa_tokenizer *t, pa_strbuf *buf, bool *fail) {
    return pa_cli_command_unload_alsa_module(c, t, buf, fail, "module-alsa-source");
}
static int pa_cli_command_unload_alsa_module(pa_core *c, pa_tokenizer *t, pa_strbuf *buf, bool *fail, const char *module_name ) {
    pa_module *m;
    uint32_t idx;
    ...
    if (!(i = pa_tokenizer_get(t, 1))) {
        pa_strbuf_puts(buf, "You need to specify the alsa sink/source name.\n");
        return -1;
    }
    ...
    if(j == NULL) {
        pa_log("%s: memory alloc failed\n", __func__);
        return -1;
    }

    pa_snprintf(j, len, "%s ", i);

    PA_IDXSET_FOREACH(m, c->modules, idx) {
        if (pa_streq(module_name, m->name)) {
            if(strstr(m->argument, j)) {
                unloaded = true;
                pa_module_unload_request(m, false);
                break;
            }
    ...
    if (unloaded == false) {
        pa_strbuf_printf(buf, "%s %s not loaded.\n", module_name, i);
        pa_xfree(j);
        return -1;
    } else {
        pa_strbuf_printf(buf, "%s %s unloaded done.\n", module_name, i);
        pa_log_debug("%s() : %s %s unloaded done.\n", __func__, module_name, i);
    }
