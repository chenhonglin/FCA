...
char *pa_get_runtime_dir(void) {
    ...
    if (d) {
        ...
        /*if dir already exist, no need create*/
        if(!access(k, R_OK|W_OK))
            return k;
...
FILE *pa_open_config_file(const char *global, const char *local, const char *env, char **result) {
    ...
    if (env && (fn = getenv(env))) {
        ...
        pa_log_warn("Failed to open configuration file '%s': %s, %s", fn, pa_cstrerror(errno), env);
        return NULL;
    }
    ...
    if (local) {
        ...
        sprintf(path_config, PA_DEFAULT_CONFIG_DIR);

		setenv("PULSE_CONFIG_PATH", path_config, 1);
