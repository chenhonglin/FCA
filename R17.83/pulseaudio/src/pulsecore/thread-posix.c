...
#ifndef ML_SCHED_PRIORITY
#define ML_SCHED_PRIORITY 50
#endif
...
static int thread_init_attr(pthread_attr_t *attr, int sched_policy, int prio)
{
    ...
    if (ret) {
        pa_log("pthread_attr_init() fail: %d", ret);
        return -1;
    }

    ret = pthread_attr_setschedpolicy(attr, sched_policy);
    if (ret) {
        pa_log("pthread_setschedpolicy() fail: %d", ret);
        return -1;
    }

    ret = pthread_attr_setschedparam(attr, &sched_param);
    if (ret) {
        pa_log("pthread_attr_setschedparam() fail: %d", ret);
        return -1;
    }
    return 0;
}
...
pa_thread* pa_thread_new(const char *name, pa_thread_func_t thread_func, void *userdata) {
    ...
    t->userdata = userdata;
    ret = thread_init_attr(&attr, SCHED_RR, ML_SCHED_PRIORITY);

    if (pthread_create(&t->id, ret==0 ? &attr : NULL, internal_thread_func, t) != 0) {
        pa_log_info("mainloop pthread create failed with priority set, try again!");
        if (pthread_create(&t->id, NULL, internal_thread_func, t) != 0) {
            pa_log("mainloop pthread create failed again, return!!!!");
            pa_xfree(t);
            return NULL;
        }
    }
