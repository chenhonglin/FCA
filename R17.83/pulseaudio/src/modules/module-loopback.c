...
PA_MODULE_USAGE(
        ...
        "sink_input_prebuf=<prebuf in bytes>");
...
static int sink_input_pop_cb(pa_sink_input *i, size_t nbytes, pa_memchunk *chunk) {
    ...
    if (pa_memblockq_peek(u->memblockq, chunk) < 0) {
        pa_log_info("Loopback %s -> %s: Could not peek into queue",
            pa_strnull(u->source_name),
...
static void sink_input_attach_cb(pa_sink_input *i) {
    ...
    if (u->sink_input_prebuf > 0)
        pa_memblockq_set_prebuf(u->memblockq, u->sink_input_prebuf);
    else
...
int pa__init(pa_module *m) {
    ...
    sink_input_prebuf = 0;
    if (pa_modargs_get_value_u32(ma, "sink_input_prebuf", &sink_input_prebuf) < 0 || sink_input_prebuf > MEMBLOCKQ_MAXLENGTH) {
        pa_log("Invalid sink_input_prebuf specification");
    }

    if (sink_input_prebuf > 0 && sink_input_prebuf < MEMBLOCKQ_MAXLENGTH)
        u->sink_input_prebuf = sink_input_prebuf;
    else
        u->sink_input_prebuf = 0;
    ...
    u->sink_input->name = pa_xstrdup(pa_proplist_gets(u->sink_input->proplist, PA_PROP_MEDIA_NAME));
    ...
    u->source_name = pa_xstrdup(u->source_output->source->name);
    u->sink_name = pa_xstrdup(u->sink_input->sink->name);
