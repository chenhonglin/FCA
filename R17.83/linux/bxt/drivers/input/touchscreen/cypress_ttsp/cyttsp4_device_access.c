...
static u16 data_read_length;
static u16 input_len;	/*Given by user for get_panel_capacitance*/
		...
		size_t length, int *size)
{
	...
	cmd_buf[5] = CYTTSP4_TCH_PARM_EBID;
		...
		u8 *ic_buf, size_t length)
{
	...
	if (!pdata->sett[CY_IC_GRPNUM_TCH_PARM_SIZE]) {
		dev_err(dev, "%s: Missing platform data Touch Parameters Sizes table\n",
			__func__);
		return -EINVAL;
	}

	if (!pdata->sett[CY_IC_GRPNUM_TCH_PARM_SIZE]->data) {
		...
		size_t length)
{
	...
	if (num_btns <= 0 || !btn || length < num_btns)
		return -EINVAL;
		...
		u8 *ic_buf, size_t length)
{
	...
	if (mode == CY_HST_CAT) {
		...
		dev_vdbg(dev, "%s: %s=%d %s=%02X %s=%d(CaT)\n", __func__,
			 "own_exclusive", dad->own_exclusive == 1,
		...
		size_t length)
{
	...
	if (dad->test.cur_cmd == CY_CMD_CAT_NULL) {
		...
		cyttsp4_pr_buf(dev, NULL, ic_buf, length, "test_cmd");
		/*
		 * 0x08 is get self data to be run afterr 07
		 */
		...
		size_t length)
{
	...
	return cyttsp4_device_access_write_config(dev, CYTTSP4_TCH_PARM_EBID,
		...
		size_t length)
{
	...
	u8 ebid = CYTTSP5_DDATA_EBID;

	if (cd->dev_id == CY_DEVID_GEN3)
		ebid = CYTTSP4_DDATA_EBID;
	/*
	 *  +4 offset is to bypass 2 byte length and 2 byte max length fields
	 */
	return cyttsp4_device_access_write_config(dev, ebid,
		...
		size_t length)
{
	struct cyttsp4_device_access_data *dad
		= cyttsp4_get_device_access_data(dev);
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	u8 ebid = CYTTSP5_MDATA_EBID;

	if (cd->dev_id == CY_DEVID_GEN3)
		ebid = CYTTSP4_MDATA_EBID;
...
static int _cyttsp4_execute_panel_scan_cmd(struct device *dev)
{
	...
	if ((cd->dev_id != CY_DEVID_GEN3) &&
	    (ret_buf[0] != CY_CMD_STATUS_SUCCESS)) {
		/*
		 * Solo does not update status register in
		 * execute panel scan command.
		 */
		rc = -EINVAL;
		goto exit;
...
static int _cyttsp4_retrieve_panel_scan_cmd(struct device *dev,
		u16 offset, u16 length, u8 data_id,
		u8 *buf, int *size)
{
 ...
 */
static int _cyttsp4_run_cap_self_test_cmd(struct device *dev,
 ...
 */
static int _cyttsp4_get_cap_self_test_results_cmd(struct device *dev,
		u8 test_id, u16 offset,
		...
		u16 *act_length, u8 *data)
{
	...
	if (data_read_length && data) {
		 ...
		 * Read test result data
		 */
		rc = cmd->read(dev, CY_MODE_CAT, CY_REG_CAT_CMD + 1, data,
				data_read_length);
		if (rc)
			goto exit;
	}
...
release_exclusive:
	...
	for (i = 0; i < dad->heatmap.num_element; i++) {
...
/*
 * TO print the out put of Cp and Cm get_panel_capacitance
 */
...
static int _cyttsp4_run_panel_test(struct device *dev,
		u8 test_type, u8 *status, u8 *summary_result)
{
	u8 test_id = CY_ST_ID_CM_PANEL_TEST + test_type;

	return _cyttsp4_run_cap_self_test_cmd(dev,
			test_id, status, summary_result, NULL);
}

static int _cyttsp4_run_button_test(struct device *dev,
		u8 test_type, u8 *status, u8 *summary_result)
{
...
static int _cyttsp4_get_panel_test_results(struct device *dev,
		...
		u8 *data, u8 grp_cap_id)
{
	...
	if ((grp_cap_id == 1) && (CY_ST_ID_CM_PANEL_TEST == 5)) {
		...
		res = _cyttsp4_get_cap_self_test_results_cmd(dev, test_id,
				pt_offset, pt_length, status, act_length, data);

		/*
		 * Header size is 5 i.e,. DATA_INFO
		 */
		pt_offset += (data_read_length - DATA_INFO);
		pt_length -= (data_read_length - DATA_INFO);
	}
...
static int _cyttsp4_get_button_test_results(struct device *dev,
		u8 test_type, u8 *status, u16 *act_length, u8 *data)
{
	u8 test_id = CY_ST_ID_CM_BUTTON_TEST + test_type;
	/* Set length to Max */
	return _cyttsp4_get_cap_self_test_results_cmd(dev, test_id,
			0, 65535, status, act_length, data);
}
/*
 * To get complete Cm/Cp Data
 */
static ssize_t cyttsp4_get_panel_capacitance_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	while (l_cnt < loop_len) {
		 ...
		 */
		if (dad->cap_target == CAP_TARGET_PANEL)
			rc = _cyttsp4_get_panel_test_results(dev,
				dad->cap_test_type, &cmd_status, &act_length,
				dad->ic_buf, 1);
		else if (dad->cap_target == CAP_TARGET_BUTTON)
			rc = _cyttsp4_get_button_test_results(dev,
				dad->cap_test_type,
...
static ssize_t cyttsp4_get_panel_capacitance_store(struct device *dev,
...
static DEVICE_ATTR(get_panel_capacitance, S_IRUSR | S_IWUSR,
	cyttsp4_get_panel_capacitance_show,
	cyttsp4_get_panel_capacitance_store);
		...
		struct cyttsp4_attribute *attr, char *buf)
{
	...
	rc = _cyttsp4_retrieve_data_structure_cmd(dev, 0, read_length,
			dad->get_idac_data_id,
			&cmd_status, &data_format,
...
static ssize_t cyttsp4_gest_show(struct device *dev,
		struct cyttsp4_attribute *attr, char *buf)
{
	struct cyttsp4_device_access_data *dad
		= cyttsp4_get_device_access_data(dev);
...
static CY_ATTR(gesture, S_IRUSR, cyttsp4_gest_show, NULL);

static ssize_t cyttsp4_noise_show(struct device *dev,
		struct cyttsp4_attribute *attr, char *buf)
{
	struct cyttsp4_device_access_data *dad
		= cyttsp4_get_device_access_data(dev);
...
static CY_ATTR(noise, S_IRUSR, cyttsp4_noise_show, NULL);
		...
		char *buf)
{
	...
	/* For TMA4xx */
	if (dad->opens_device_type == DEVICE_TYPE_TMA4xx)
		rc = _cyttsp4_get_opens_self_test_results_tma4xx
			(dev,
			 dad->opens_test_type,
			 &cmd_status,
			 &act_length,
...
static ssize_t cyttsp4_capacitance_show(struct device *dev,
		struct cyttsp4_attribute *attr,
		char *buf)
{
	struct cyttsp4_device_access_data *dad
	...
	/* Run capacitance Self Test */
	if (dad->cap_target == CAP_TARGET_PANEL)
		rc = _cyttsp4_run_panel_test(dev, dad->cap_test_type,
				&cmd_status, &summary_result);
	else if (dad->cap_target == CAP_TARGET_BUTTON)
		rc = _cyttsp4_run_button_test(dev, dad->cap_test_type,
				&cmd_status, &summary_result);
	else
		rc = -EINVAL;
	...
	/* Get capacitance Self Test Results */
	if (dad->cap_target == CAP_TARGET_PANEL)
		rc = _cyttsp4_get_panel_test_results(dev, dad->cap_test_type,
				&cmd_status, &act_length,
				dad->ic_buf, 0);
	else if (dad->cap_target == CAP_TARGET_BUTTON)
		rc = _cyttsp4_get_button_test_results(dev, dad->cap_test_type,
				&cmd_status, &act_length,
				dad->ic_buf);
	else
		rc = -EINVAL;
...
static ssize_t cyttsp4_capacitance_store(struct device *dev,
		struct cyttsp4_attribute *attr,
		const char *buf, size_t size)
{
	struct cyttsp4_device_access_data *dad
...
static CY_ATTR(capacitance, S_IRUSR | S_IWUSR,
		cyttsp4_capacitance_show, cyttsp4_capacitance_store);
...
static int cyttsp4_setup_sysfs(struct device *dev)
{
	...
	if (cd->dev_id & GEN6_DEVICE) {
		rc = cyttsp4_create_file(dev, &cy_attr_capacitance);
		i = 1;
		if (rc) {
	...
	rc = cyttsp4_create_file(dev, &cy_attr_gesture);
	if (rc) {
		dev_err(dev, "%s: Error, could not create baseline\n",
			__func__);
	...
	rc = cyttsp4_create_file(dev, &cy_attr_noise);
	if (rc) {
		dev_err(dev, "%s: Error, could not create baseline\n",
			__func__);
...
unregister_gesture:
	cyttsp4_remove_file(dev, &cy_attr_gesture);
unregister_baseline:
...
unregister_capacitance:
	if (cd->dev_id & GEN6_DEVICE)
		cyttsp4_remove_file(dev, &cy_attr_capacitance);
unregister_opens:
...
static void cyttsp4_device_access_command_work_func(struct work_struct *work)
{
	struct command_work *cmd_work = container_of(work, struct command_work,
			work);
	int rc;
...
static int cyttsp4_device_access_command_async(const char *core_name,
		...
		u8 *buf, int length, void (*cont)
		(const char *core_name,
		 int ic_grpnum, int ic_grpoffset,
		 u8 *buf, int length, int rc),
		bool read)
{
...
static int cyttsp4_device_access_release(struct device *dev)
{
	...
	if (dad->own_exclusive) {
		dev_err(dev,
			"%s: Can't unload in CAT mode. 1st switch back to Operational mode\n",
			__func__);
	...
	if (dad->sysfs_nodes_created) {
		...
		if (cd->dev_id & GEN6_DEVICE) {
			cyttsp4_remove_file(dev, &cy_attr_gesture);
			cyttsp4_remove_file(dev, &cy_attr_noise);
		}
		kobject_put(&dad->mfg_test);
	} else {
		cmd->unsubscribe_attention(dev, CY_ATTEN_STARTUP,
