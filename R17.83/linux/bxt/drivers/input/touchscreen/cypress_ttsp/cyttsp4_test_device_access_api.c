...
static char *core_name = CY_DEFAULT_CORE_ID;

module_param(core_name, charp, 0);
MODULE_PARM_DESC(core_name,
		 "Core name of cyttsp4 core device for test device access api");
...
static int __init cyttsp4_test_device_access_api_init(void)
{
	 ...
	 */
	rc = cyttsp4_device_access_read_command(core_name,
						GRPNUM_TOUCH_CONFIG,
						CONFIG_VER_OFFSET,
						buffer, BUFFER_SIZE);
	if (rc < 0) {
	 ...
	 */
	rc = cyttsp4_device_access_write_command(core_name,
						 GRPNUM_TOUCH_CONFIG,
	...
	preempt_disable();
	rc =
	cyttsp4_device_access_write_command_async(core_name,
						  GRPNUM_OP_COMMAND, 0,
						  buffer, 4,
