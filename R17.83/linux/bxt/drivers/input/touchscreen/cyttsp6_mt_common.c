...
extern unsigned int cy_native_x_pos;
extern unsigned int cy_native_y_pos;
extern unsigned int cy_native_z_pos;
...
{
	if (event_path_global) {
		/*Syfs variables update to notify native app*/
		if (event == CY_ABS_X_OST)
			cy_native_x_pos = value;
		else if (event == CY_ABS_Y_OST){
			cy_native_y_pos = value;
			update_native_touch_coordinates(cy_native_x_pos, cy_native_y_pos, cy_native_z_pos);
		}
		else if (event == CY_ABS_P_OST)
			cy_native_z_pos = value;
	} else {
		/* Default Android notify mechanism */
...
static void cyttsp6_get_mt_touches(struct cyttsp6_mt_data *md, int num_cur_rec)
{
	...
	for (i = 0; i < num_cur_rec; i++) {
		...
		if (tch.abs[CY_TCH_E] == CY_EV_LIFTOFF) {
			...
			/*To send BUTTON TOUCH (0x14a) event on liftoff*/
		...
		for (j = 0; j <= CY_ABS_W_OST ; j++)
			cyttsp6_report_event(md, CY_ABS_X_OST + j,
					tch.abs[CY_TCH_X + j]);
		/*BUTTON TOUCH (0x14a) event when single finger touch down is detected*/
		if(CY_EV_TOUCHDOWN == tch.abs[CY_TCH_E])
