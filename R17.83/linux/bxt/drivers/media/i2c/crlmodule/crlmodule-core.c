...
#if defined (CONFIG_INTEL_IPU4_AR0143_RVC) || defined (CONFIG_INTEL_IPU4_AR0143_AVM)
#include "ar0143-reg.h"
#endif


#ifdef CONFIG_INTEL_IPU4_DS90UB954_MOPAR
#define GET_LIMITS(s) ((s->sensor_ds->get_limits \
	? s->sensor_ds->get_limits(v4l2_get_subdevdata(&s->src->sd)) \
	: s->sensor_ds->sensor_limits))

#define GET_MODES(s,m,mi) do{\
		if(s->sensor_ds->get_modes) { \
			s->sensor_ds->get_modes(v4l2_get_subdevdata(&s->src->sd),&m, &mi);\
		}else{\
			m=s->sensor_ds->modes; \
			mi=s->sensor_ds->modes_items; \
		} \
	} while(0)
#else
#define GET_LIMITS(s) (s->sensor_ds->sensor_limits)
#define GET_MODES(s,m,mi) do { m=s->sensor_ds->modes; mi=s->sensor_ds->modes_items; } while(0)
#endif
...
extern int max9296_check_sync_status(void);
#endif
					  ...
					  u32 code)
{
	...
	for (i = 0; i < sensor->sensor_ds->csi_fmts_items; i++) {
               printk("sensor->sensor_ds->csi_fmts[i].code =%x, code =%x \n",sensor->sensor_ds->csi_fmts[i].code,code);
		if (sensor->sensor_ds->csi_fmts[i].code == code) {
			printk("Matching code = %x\n",code);
			return i;
		}
	}
...
{
	struct i2c_client *client = v4l2_get_subdevdata(&sensor->src->sd);
	struct crl_mode_rep *mode = sensor->current_mode;
	unsigned int val;
	int ret;
				...
				struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_NATIVE_SIZE:
		if (ssd == sensor->pixel_array) {
			...
			sel->r.width = limits->x_addr_max;
			sel->r.height = limits->y_addr_max;
...
static int crlmodule_start_streaming(struct crl_sensor *sensor)
{
	...
	if (sensor->sensor_ds->streamon_callback) {
		rval = sensor->sensor_ds->streamon_callback(client);
		if (rval) {
			dev_err(&client->dev, "%s failed to set stream: %d\n", __func__, rval);
			return rval;
		}
	}
...
{
	int rval;
	if (sensor->sensor_ds->streamoff_callback) {
		struct i2c_client *client =
			v4l2_get_subdevdata(&sensor->src->sd);
		rval = sensor->sensor_ds->streamoff_callback(client);
		if (rval) {
			dev_err(&client->dev, "%s failed to set stream-off: %d\n", __func__, rval);
			return rval;
		}
	}
...
{
    printk("About to Wait for 10msec before we write power on registers");

    usleep_range(10000, 10000 + 10);

    printk("Writing Power On Registers");
    ...
    printk("Poweron registers are updated");
...
static int crlmodule_init_subdevs(struct v4l2_subdev *subdev)
{
	...
	for (i = 0; i < sensor->ssds_used; i++) {
		...
		sd->sink_fmt.width = limits->x_addr_max;
		sd->sink_fmt.height = limits->y_addr_max;
...
static int crlmodule_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	for (i = 0; i < ssd->npads; i++) {
		...
		try_fmt->width = limits->x_addr_max;
		try_fmt->height = limits->y_addr_max;
...
static int crlmodule_suspend(struct device *dev)
{
	...
	if (sensor->sensor_ds->suspend_callback)
		sensor->sensor_ds->suspend_callback(client);
...
static int crlmodule_resume(struct device *dev)
{
	...
	if (sensor->power_count > 0) {
		...
		rval = crlmodule_write_regs(sensor,
				sensor->sensor_ds->powerup_regs,
				sensor->sensor_ds->powerup_regs_items);
		if (rval != 0)
			dev_err(&client->dev, "%s failed to power up sensor\n",
						__func__);
	}

	if (sensor->sensor_ds->resume_callback)
		sensor->sensor_ds->resume_callback(client);

	if (!rval && sensor->streaming)
		rval = crlmodule_start_streaming(sensor);

#ifdef CONFIG_INTEL_IPU4_DS90UB954_MOPAR
	/* Initilizing Deserializer DS90UB954 again with
	   set of values since it reset in suspend cycle
	   */
        rval = crlmodule_write_regs(sensor,
			sensor->sensor_ds->onetime_init_regs,
			sensor->sensor_ds->onetime_init_regs_items);
	if (rval) {
		dev_err(&client->dev, "%s failed to set onetimeinit registers\n",
				__func__);
		rval = -ENODEV;
	}
...
#define NUM_MAX_CAMERAS 4
struct crl_sensor *sensor_array[NUM_MAX_CAMERAS]; /* To use in below function */
int check_camera_status(int *camera_status, int camera_type)
{
        u32 val;
        int i, ret = 0;
	struct crl_sensor *sensor=NULL;
	const char * sensor_name = NULL;

	/* Find sensor that matches camera type (by name) */
	for (i=0; i < ARRAY_SIZE(sensor_array) && !sensor; i++){
		sensor_name = "";
		if (sensor_array[i])
			sensor_name = crlmodule_sensor_name(sensor_array[i]);
		/* camera_type: 0=analog, 1=digital(1280x800), 2=digital(1024x768) */
		if (camera_type==0 && !strcmp("adv7281_cvbs", sensor_name)){
			sensor = sensor_array[i];
		} else if( (camera_type == 1 || camera_type == 2) &&
				!strcmp("ds90ub954", sensor_name)) {
			sensor = sensor_array[i];
		} else if (camera_type == 0 && !strcmp("aptina_rvc", sensor_name)) {
			sensor = sensor_array[i];
		}
	}
	if (sensor){
		struct i2c_client *client = v4l2_get_subdevdata(&(sensor->src->sd));
		/* Sensors should preferably implement their own status
		 * functions */
		if ( sensor->sensor_ds->sensor_status ) {
			return sensor->sensor_ds->sensor_status(client, camera_status);
		}
#ifdef CONFIG_INTEL_IPU4_APTINA_RVC
		if (camera_type == 0 && !strcmp("aptina_rvc", sensor_name)) {
			int status = max9296_check_sync_status();
			printk("camera status = %d\n", status);
			*camera_status = status;
			ret = 0;
		} else
#endif
                {
		pr_debug("%s: got sensor for camera_type=%d @index=%d\n", __func__,
			camera_type, i);

		for (i = 0; i < sensor->sensor_ds->status_regs_items; i++) {
			ret = crlmodule_read_reg(sensor,
						sensor->sensor_ds->status_regs[i],
						&val);
			if (ret) {
				dev_err(&client->dev, "camera_status: status register read failed\n");
				break;
			}

			if (camera_type == 0 && sensor->sensor_ds->status_regs[i].address == 0x04) {
				*camera_status = ((val & 0x80) == 0x80) ? 1 : 0;
			}
			/* For DS90UB954, set camera_status<0> = LOCK & PASS.
			   Requires reg 0x0C to be set appropriately. */
			if ( (camera_type==1 || camera_type==2) &&
					sensor->sensor_ds->status_regs[i].address == 0x04) {
				*camera_status = ((val & 0x6)==0x6) ? 1 : 0;
			}

			dev_dbg(&client->dev, "Status Register = 0x%x"
						" Value = 0x%x\n",
						sensor->sensor_ds->status_regs[i].address,
						val);
		}
		}
	} else {
		pr_info("%s: no sensor found for camera_type=%d\n", __func__, camera_type);
		ret = -ENODEV;
	}

       return ret;
}
EXPORT_SYMBOL(check_camera_status);
			   ...
			   const struct i2c_device_id *devid)
{
	...
	sensor_array[sensor_index] = sensor;
        sensor_index++;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 5, 0)
...
static __init int init_crlmodule(void)
{
	return i2c_add_driver(&crlmodule_i2c_driver);
}

static __exit void exit_crlmodule(void)
{
	i2c_del_driver(&crlmodule_i2c_driver);
}
fs_initcall_sync(init_crlmodule);
module_exit(exit_crlmodule);
#else
