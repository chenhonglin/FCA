...
#define MOAL_CMD_TIMEOUT                20
	do_gettimeofday(&t);
	if (info.pending_cmd && info.dnld_cmd_in_secs &&
...
woal_store_firmware_dump(moal_handle *phandle, mlan_event *pmevent)
{
	...
	phandle->fw_dump_len += pmevent->event_len;
	memcpy(pos, pmevent->event_buf, pmevent->event_len);
	PRINTM(MEVENT, "fw dump event: evt_len=%d toal_len=%ld\n",
...
*/
int
woal_save_dump_info_to_buf(moal_handle *phandle, t_u8 *src, t_u32 len,
			   t_u32 type)
{
	...
	while (left_len) {
		...
		memcpy(dest, &header, sizeof(mem_dump_header));
...
woal_append_end_block(moal_handle *phandle)
{
	...
	header.len = 0;
	memcpy(pos, &header, sizeof(mem_dump_header));
