/*
 * drivers/video/bjev_ivi_display.c
 * FPDLink Serializer/Deserializer  for bjev ivi display driver
 */
...
struct i2c_client *ivi_ser_global_client;


struct bjev_ivi_display_rec {
	struct bjev_ivi_display_platform_data *pdata;
...
extern void bjev_ivi_ser_read_isr(void)
{
	...
	err = ser_i2c_read(ivi_ser_global_client, SER_REG_INT_RECEIVE, &val);
	if (err < 0)
		dev_err(&ivi_ser_global_client->dev, "INT reciever 0xC7 read fail err=%d\n", err);
	dev_dbg(&ivi_ser_global_client->dev, "INT reciever 0xC7: 0x%u\n", val);
}

EXPORT_SYMBOL_GPL(bjev_ivi_ser_read_isr);
...
static void register_mcu_touch_controller(struct work_struct *work_arg)
{
	...
	/* Then the bjev_ivi_display_rec pointer */
	struct bjev_ivi_display_rec *data =
		container_of(delayed_work, struct bjev_ivi_display_rec, work_item);

	pr_notice("bjev_ivi_display: delay complete - starting mcu and touch driver\n");
#if 0
...
static void check_lock_status(struct work_struct *work_arg)
{
	...
	if (err == 1) {
		pr_notice("bjev_ivi_display: LVDS_LOCK detected, starting delay\n");
...
static int bjev_ivi_display_enable(struct i2c_client *client)
{
	struct bjev_ivi_display_rec *data = i2c_get_clientdata(client);
	int err;
	uint8_t val;

	dev_err(&client->dev, "bjev_ivi_display_enable: entry\n");
/* Modify Ian */
//#if CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_CORE
	memset(&data->touch2_device_board_info, 0, sizeof(data->touch2_device_board_info));
	strlcpy(data->touch2_device_board_info.type, "synaptics_dsx_i2c_ivi", I2C_NAME_SIZE);
...
//#endif
//#if CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4
	memset(&data->touch1_device_board_info, 0, sizeof(data->touch1_device_board_info));
	strlcpy(data->touch1_device_board_info.type, "cyttsp6_i2c_adapter", I2C_NAME_SIZE);
...
#if 0
	if (devm_gpio_request_one(&client->dev, SER_PDB_GPIO, 0, "SERIALIZER PDB") != 0) {
...
//#if CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4
		/* Create work queue */
		data->work_queue = create_singlethread_workqueue("bjev_ivi_display");
...
//#endif
	...
	if (err < 0) {
		pr_err("bjev_ivi_display_enable: i2c read/write errir!\n");
		return err;
	} else {
/*		if (data->touch_client) {
			pr_notice("bjev_ivi_display:Releasing touch client\n");
			i2c_release_client(data->touch_client);
			data->touch_client = 0;
		}
*/
		pr_notice("bjev_ivi_display:Report success\n");
		return 0;
	}
}

static void bjev_ivi_display_disable(struct i2c_client *client)
{
	struct bjev_ivi_display_rec *data = i2c_get_clientdata(client);
#if 0
	if (data->touch_client) {
...
static void release_all_resources(struct bjev_ivi_display_rec *data)
{
	if (data->work_queue) {
...
static int bjev_ivi_display_probe(struct i2c_client *client,
				const struct i2c_device_id *id)
{
	...
	int err;
	pr_notice("bjev_ivi_display_probe: success\n");
	dev_err(&client->dev, "bjev_ivi_display_probe: success\n");

	ivi_ser_global_client = client;
	...
	err = bjev_ivi_display_enable(client);
	if (err < 0) {
...
static int bjev_ivi_display_remove(struct i2c_client *client)
{
	struct bjev_ivi_display_rec *data = i2c_get_clientdata(client);
	bjev_ivi_display_disable(client);
	i2c_set_clientdata(client, NULL);
	return 0;
}

#ifdef CONFIG_PM_SLEEP
static int bjev_ivi_display_suspend(struct device *dev)
{
...
static int bjev_ivi_display_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	return bjev_ivi_display_enable(client);
}
#endif

static const struct i2c_device_id bjev_ivi_display_id[] = {
	{ "bjev_ivi_display", 0 },
	{ }
};

MODULE_DEVICE_TABLE(i2c, bjev_ivi_display_id);

static SIMPLE_DEV_PM_OPS(bjev_ivi_display_pm_ops, bjev_ivi_display_suspend, bjev_ivi_display_resume);

static struct i2c_driver bjev_ivi_display_driver = {
	.driver = {
		.name  = "bjev_ivi_display",
		.owner = THIS_MODULE,
		.pm	   = &bjev_ivi_display_pm_ops,
	},
	.probe    = bjev_ivi_display_probe,
	.remove   = bjev_ivi_display_remove,
	/*.shutdown = bjev_ivi_display_remove,*/
	.id_table = bjev_ivi_display_id,
};

module_i2c_driver(bjev_ivi_display_driver);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("bjev_ivi_display FPDLink Serializer driver");
MODULE_ALIAS("i2c:bjev_ivi_display");
