...
int intel_pinctrl_suspend(struct device *dev)
{
	...
	for (i = 0; i < pctrl->soc->npins; i++) {
		...
		/***
		 * Set GPIO_20, the i210 springville chip's PERST / pcie reset
		 * line to low state (disable TX) before suspend.
		 *
		 * This is the last place in the kernel to touch padcfg
		 * registers, so its guaranteed to work... but it might be
		 * better to do this from PCIE driver or possibly igb_avb
		 * driver. gpio-dv1-s2r suspend occurs *BEFORE* PCIE suspend
		 * in current impl hence NOT putting it there.
		 */
		if ( desc->number == 20 && desc->name && strncmp(desc->name, "GPIO_20", 7) == 0){
			void __iomem *padcfg0 = intel_get_padcfg(pctrl, desc->number, PADCFG0);
			u32 newval;
			val = readl(padcfg0);
			newval = ((val | PADCFG0_GPIOTXDIS) & (~PADCFG0_GPIOTXSTATE));
			writel(newval, padcfg0);
			dev_info(dev, "%s:GPIO_20 DW0 0x%08x => 0x%08x\n", __func__, val, newval);
		}
...
int intel_pinctrl_resume(struct device *dev)
{
	...
	for (i = 0; i < pctrl->soc->npins; i++) {
		...
		/*** Reverse the GPIO_20 change that was done on suspend.  */
		if ( desc->number == 20 && desc->name && strncmp(desc->name, "GPIO_20", 7) == 0){
			void __iomem *padcfg0 = intel_get_padcfg(pctrl, desc->number, PADCFG0);
			val = readl(padcfg0);
			if ( (val & PADCFG0_GPIOTXDIS) || ((val & PADCFG0_GPIOTXSTATE)==0) ) {
				u32 newval = val;
				newval &= ~PADCFG0_GPIOTXDIS;
				newval |= PADCFG0_GPIOTXSTATE;
				writel(val, padcfg0);
				dev_info(dev, "%s:GPIO_20 DW0 0x%08x => 0x%08x\n", __func__, val, newval);
			}
		}
