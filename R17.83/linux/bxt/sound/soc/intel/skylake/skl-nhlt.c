...
int skl_nhlt_update_topology_bin(struct skl *skl)
{
	...
	board_name = harman_get_board_name();
	if((strncmp(board_name,"V2HT_X110",9) == 0)||(strncmp(board_name,"V2HT_X139",9) == 0)|| (strncmp(board_name,"V2HT_X140",9) == 0))
	{
		snprintf(skl->tplg_name, sizeof(skl->tplg_name), "%x-%.6s-%.8s-%d%s",
		skl->pci_id, nhlt->header.oem_id, nhlt->header.oem_table_id,
		nhlt->header.oem_revision, "-gwmv2mh-external-amp-tplg.bin");
	}
   else
   {
		snprintf(skl->tplg_name, sizeof(skl->tplg_name), "%x-%.6s-%.8s-%d%s",
		skl->pci_id, nhlt->header.oem_id, nhlt->header.oem_table_id,
		nhlt->header.oem_revision, "-gwmv2mh-internal-amp-tplg.bin");
	}
#elif defined(CONFIG_SND_SOC_BJEVN60_ALSA)
        const char *variant_status;
        variant_status = harman_get_A2B_variant();
        if((strncmp(variant_status,"Yes",3) == 0))
        {
                dev_dbg(dev, "A2B variant is %.6s\n",variant_status);
                snprintf(skl->tplg_name, sizeof(skl->tplg_name), "%x-%.6s-%.8s-%d%s",
                skl->pci_id, nhlt->header.oem_id, nhlt->header.oem_table_id,
                nhlt->header.oem_revision, "-bjeved2-external-amp-tplg.bin");
        }else{
                dev_dbg(dev, "A2B variant is %.6s\n",variant_status);
                snprintf(skl->tplg_name, sizeof(skl->tplg_name), "%x-%.6s-%.8s-%d%s",
                skl->pci_id, nhlt->header.oem_id, nhlt->header.oem_table_id,
                nhlt->header.oem_revision, "-bjeved2-internal-amp-tplg.bin");

        }
#elif defined(CONFIG_SND_SOC_FCA_ALSA)
	const char *variant_id;
	variant_id = get_info_audiosystem_conf();
	dev_warn(dev, "can variant is %s\n", variant_id);
	if(strcmp(variant_id, "0") == 0) {
		snprintf(skl->tplg_name, sizeof(skl->tplg_name), "%x-%.6s-%.8s-%d%s",
			skl->pci_id, nhlt->header.oem_id,
			nhlt->header.oem_table_id,
			nhlt->header.oem_revision,
			"-fcadv2-internal-amp-tplg.bin");
	}else {
		snprintf(skl->tplg_name, sizeof(skl->tplg_name), "%x-%.6s-%.8s-%d%s",
			skl->pci_id,
			nhlt->header.oem_id,
