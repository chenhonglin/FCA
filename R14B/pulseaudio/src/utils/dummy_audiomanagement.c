...
static void command_status_callback(void *userdata, unsigned char result)
{
...
int main(int argc, char *argv[])
{
    ...
    if(wait_event_back){
        pa_ac_set_event_callback(s, event_callback);
    }
    ...
    event_pending = 1;
    pa_ac_send_command(s, &msg, (cmd_status_cb_t)command_status_callback);

    /*wait for msg send complete*/
    while(s->operation_success != 0x01){
        usleep(1000);
    }
