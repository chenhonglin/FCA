...
static void unload_module_by_sink_name_callback(pa_context *c, const pa_module_info *i, int is_last, void *userdata) {
    static bool unloaded = false;
    ...
    if (pa_streq(module_name, i->name)&&(strstr(i->argument, sink_name))) {
...
static void set_sink_formats(pa_context *c, uint32_t sink, const char *str) {
    pa_format_info *f_arr[MAX_FORMATS] = {NULL};
    char *format = NULL;
        ...
        case PA_CONTEXT_READY:
            switch (action) {
                ...
                case UNLOAD_MODULE:
                    ...
                    break;
                case UNLOAD_SINK_MODULE:
                        o = pa_context_get_module_info_list(c, unload_module_by_sink_name_callback, NULL);
                    break;
                case SUSPEND_SINK:
                    if (sink_name)
                ...
                case SET_SOURCE_AND_OUTPUT_MUTE:
                        o = pa_context_mute_source_and_output_by_name(c, source_name, output_name, mute, simple_callback, NULL);
                    break;
...
static void help(const char *argv0) {
    ...
    printf("%s %s %s %s\n", argv0, _("[options]"), "unload-sink-module ", _("Module SINK "));
    ...
    printf("%s %s %s %s\n", argv0, _("[options]"), "set-source-and-output-mute", _("source_name output_name|all 1|0"));
...
int main(int argc, char *argv[]) {
    ...
    if (optind < argc) {
        ...
        } else if (pa_streq(argv[optind], "unload-sink-module")) {
            action = UNLOAD_SINK_MODULE;
            if (argc != optind+3) {
                pa_log(_("You have to specify a module name and sink name"));
                goto quit;
            }

            module_name = pa_xstrdup(argv[optind+1]);
            sink_name = pa_xstrdup(argv[optind+2]);
        ...
        } else if (pa_streq(argv[optind], "set-source-and-output-mute")) {
            action = SET_SOURCE_AND_OUTPUT_MUTE;

            if (argc != optind+4) {
                pa_log(_("You have to specify a source name and output source name and a mute boolean"));
                goto quit;
            }

            if ((mute = parse_mute(argv[optind+3])) == INVALID_MUTE) {
                pa_log(_("Invalid mute specification"));
                goto quit;
            }

            output_name = pa_xstrdup(argv[optind+2]);
            source_name = pa_xstrdup(argv[optind+1]);
