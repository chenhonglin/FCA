...
static int create_stream(
    ...
    PA_CHECK_VALIDITY(s->context, !(flags & ~(PA_STREAM_START_CORKED|
                                              ...
                                              PA_STREAM_NO_MUTE_ALWAYS)), PA_ERR_INVALID);
    ...
    if (s->direction == PA_STREAM_RECORD)
        pa_tagstruct_put_boolean(t, flags & (PA_STREAM_NO_MUTE_ALWAYS));
...
int pa_stream_write_ext_free(
    ...
    if (s->write_memblock) {
        ...
        pa_pstream_send_memblock(s->context->pstream, s->channel, offset, seek, &chunk, &s->sample_spec);
        ...
        while (t_length > 0) {
            ...
            if (free_cb && !pa_pstream_get_shm(s->context->pstream)) {
                ...
                bsm = pa_mempool_block_size_max(s->context->mempool);
                frame_size = pa_frame_size(&s->sample_spec);
                //frame_size = s ? pa_frame_size(&s->sample_spec) : 0;
                if (frame_size)
                    bsm -= bsm % frame_size;

                chunk.length = PA_MIN(t_length, bsm);
            ...
            pa_pstream_send_memblock(s->context->pstream, s->channel, t_offset, t_seek, &chunk, &s->sample_spec);
