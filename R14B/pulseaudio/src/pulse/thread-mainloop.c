...
#endif
#if 0
    /*set the sched type and priority first*/
    struct sched_param param;
    param.sched_priority = ML_SCHED_PRIORITY;
    sched_setscheduler(0, SCHED_RR, &param);
#endif
    /* set mainloop thread priority */
    if(m->rtprio >= 0)
        pa_make_realtime(m->rtprio);
...
void pa_threaded_mainloop_set_priority(pa_threaded_mainloop *m, long rtprio) {
    pa_assert(m);
