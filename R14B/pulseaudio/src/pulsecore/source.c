...
struct pa_source_spec_mute_dev {
    char *mute_dev_name;
    PA_LLIST_FIELDS(pa_source_spec_mute_dev);
};
...
pa_source_spec_mute_dev *pa_source_check_specified_mute_device_existed(pa_source *s, const char *output_name) {
    pa_assert(s);
    pa_assert(output_name);

    pa_source_spec_mute_dev *dev = NULL;
    pa_source_spec_mute_dev *found = NULL;

    PA_LLIST_FOREACH(dev, s->spec_mute_devices) {
        if (strstr(output_name, dev->mute_dev_name)) {
            pa_log_debug("source output %s existed in specified mute list!", output_name);
            found = dev;
            break;
        }
    }
...
pa_source* pa_source_new(
    ...
    s->output_mute_mode = false;
    PA_LLIST_HEAD_INIT(pa_source_spec_mute_dev, s->spec_mute_devices);
...
static void source_free(pa_object *o) {
    ...
    while ((dev = s->spec_mute_devices)) {
        PA_LLIST_REMOVE(pa_source_spec_mute_dev, s->spec_mute_devices, dev);
        pa_xfree(dev->mute_dev_name);
        pa_xfree(dev);
    }
...
void pa_source_set_volume(
    ...
    //init new_reference_volume default value
    new_reference_volume.channels = PA_CHANNELS_MAX;
    for (int i = 0; i < PA_CHANNELS_MAX; i++)
        new_reference_volume.values[i] = PA_VOLUME_MAX;
...
/* Called from main thread */
void pa_source_set_mute_source_and_output(pa_source *s, char *output_name, bool mute) {
    pa_assert(s);
    pa_assert(output_name);
    ...
    /* if all devices mute set */
    if (strncmp(output_name, "all", 3) == 0) {
        s->output_mute_mode = mute;
        pa_log_debug("Set all outputs of source %s to %s mode", s->name, mute ? "mute" : "unmute");
        return;
    }
    /* specified device mute set */
    dev = pa_source_check_specified_mute_device_existed(s, output_name);
    if (dev) {
        if (!mute) {
            PA_LLIST_REMOVE(pa_source_spec_mute_dev, s->spec_mute_devices, dev);
            pa_log_debug("source output %s has been removed from spec mute list!", dev->mute_dev_name);
        }
    } else {
        if (mute) {
            dev = pa_xnew0(pa_source_spec_mute_dev, 1);
            dev->mute_dev_name = pa_xstrdup(output_name);

            PA_LLIST_INIT(pa_source_spec_mute_dev, dev);
            PA_LLIST_PREPEND(pa_source_spec_mute_dev, s->spec_mute_devices, dev);
            pa_log_debug("source output %s has been added into spec mute list!", output_name);
        }
    }

    /* if output opened */
    PA_IDXSET_FOREACH(o, s->outputs, idx) {
        if (strstr(pa_strnull(pa_proplist_gets(o->proplist, PA_PROP_MEDIA_NAME)), output_name)) {
            o->specified_mute = mute;
            pa_log_debug("Set specified outputs %s of source %s to %s mode", output_name, s->name, mute ? "mute" : "unmute");
            break;
        }
    }
}
