...
static const pa_pdispatch_cb_t command_table[PA_COMMAND_MAX] = {
    ...
    [PA_COMMAND_MUTE_SOURCE_AND_OUTPUT] = command_set_mute,
...
static playback_stream* playback_stream_new(
    ...
    *ret = -pa_sink_input_new(&sink_input, c->protocol->core, &data);
    ...
    if (!sink_input)
        goto out;
    sink_input->name = pa_xstrdup(pa_proplist_gets(p, PA_PROP_MEDIA_NAME));
...
static void native_connection_send_memblock(pa_native_connection *c) {
    ...
    for (;;) {
        ...
        if (pa_memblockq_peek(r->memblockq, &chunk) >= 0) {
            ...
            pa_pstream_send_memblock(c->pstream, r->index, 0, PA_SEEK_RELATIVE, &schunk, &r->source_output->sample_spec);
...
/*         pa_log("%lu vs. %lu", (unsigned long) pa_memblockq_get_length(s->memblockq), (unsigned long) pa_memblockq_get_prebuf(s->memblockq)); */
        ...
        if (pa_memblockq_is_readable(s->memblockq)) {
            ...
            if (s->sink_input->thread_info.underrun_for == (uint64_t) -1)
                pa_log_debug("Stream starting end of prebuf : %s ",pa_strnull(pa_proplist_gets(s->sink_input->proplist, PA_PROP_MEDIA_NAME)));
            else
                pa_log_debug("Requesting rewind due to end of underrun : %s ",pa_strnull(pa_proplist_gets(s->sink_input->proplist, PA_PROP_MEDIA_NAME)));
...
static int sink_input_process_msg(pa_msgobject *o, int code, void *userdata, int64_t offset, pa_memchunk *chunk) {
    ...
    switch (code) {
        ...
        case SINK_INPUT_MESSAGE_UPDATE_LATENCY:
            ...
            s->current_sink_latency = 0;
            /*use 0 instead of pa_sink_get_latency_within_thread(s->sink_input->sink).
            Because pa_sink_get_latency_within_thread may return a large latency when any of alsa device has problem.
            Such latency may cause app stop feeding data and logical device underrun. PA only update logical device latency to app when use 0.*/
...
static int sink_input_pop_cb(pa_sink_input *i, size_t nbytes, pa_memchunk *chunk) {
    ...
    if (pa_memblockq_peek_fixed_size(s->memblockq, nbytes, chunk) < 0)
        return -1;
...
#ifdef DISABLE_REFINE_TLENGTH
    return;		/*not update tlength*/
#endif
...
static void command_create_record_stream(pa_pdispatch *pd, uint32_t command, uint32_t tag, pa_tagstruct *t, void *userdata) {
    ...
    if (pa_tagstruct_get_boolean(t, &no_mute) < 0) {
        protocol_error(c);
        goto finish;
    }
    ...
    flags =
        ...
        (no_mute ? PA_SOURCE_OUTPUT_NO_MUTE_ALWAYS : 0);
...
static void setup_srbchannel(pa_native_connection *c) {
    ...
    pa_pstream_send_memblock(c->pstream, 0, 0, 0, &mc, NULL);
...
static void command_set_mute(
    ...
    pa_sink_input *si = NULL;
    pa_source_output *so = NULL;
    const char *name = NULL, *subname = NULL, *client_name;
    ...
    if (pa_tagstruct_getu32(t, &idx) < 0 ||
        ...
        (command == PA_COMMAND_MUTE_SOURCE_AND_OUTPUT && pa_tagstruct_gets(t, &name) < 0) ||
        (command == PA_COMMAND_MUTE_SOURCE_AND_OUTPUT && pa_tagstruct_gets(t, &subname) < 0) ||
    ...
    switch (command) {
        ...
        case PA_COMMAND_MUTE_SOURCE_AND_OUTPUT:
            if (idx != PA_INVALID_INDEX)
                source = pa_idxset_get_by_index(c->protocol->core->sources, idx);
            else
                source = pa_namereg_get(c->protocol->core, name, PA_NAMEREG_SOURCE);
    ...
    if (sink) {
        ...
        if (subname != NULL) {
            pa_log_debug("Client %s changes mute of output %s in source %s.", client_name, subname, source->name);
            pa_source_set_mute_source_and_output(source, subname, mute);
        } else {
...
static void pstream_die_callback(pa_pstream *p, void *userdata) {
    ...
    /* it's normal trace after a stream disconnecting, word "died" easy to consider
       as something failed, change it to "terminated" is more reasonable */
    //pa_log_info("Connection died.");
    pa_log_info("Connection terminated.");
}
