...
#ifdef HAVE_CONFIGURABLE_CROSSBAR_LIB
#include "../utils/EarlyPersistHalHelper.h"
#endif
...
#define MEMBLOCKQ_MAXTIME 2000000
...
#define INPUT_CH_MAX   MAX_MIX_CHANNELS
#define INPUT_SRC_MAX  INPUT_CH_MAX
#define OUTPUT_SINK_MAX   (32)

struct crossbar_input {
        char *name;
        unsigned int offset;
        unsigned int channels;
        unsigned int id;
        PA_LLIST_FIELDS(struct crossbar_input);
};

struct crossbar_output {
        char *name;
        unsigned int id;
        unsigned int offset;
        unsigned int channels;
        unsigned int external_trigger;
        PA_LLIST_FIELDS(struct crossbar_output);
};
...
struct userdata {
    ...
    int input_dump_fd[INPUT_SRC_MAX];
    int output_dump_fd[OUTPUT_SINK_MAX];
    int8_t *dump_buf;
    bool dumping;
    ...
    struct {
        PA_LLIST_HEAD(struct crossbar_input, crossbar_config_inputs);
        PA_LLIST_HEAD(struct crossbar_output, crossbar_config_outputs);
        int crossbar_input_channels;
        int crossbar_input_count;
        int crossbar_output_channels;
        int crossbar_output_count;
        char *crossbar_libpath;
    } crossbar_config_info;
...
static char *strdup_remove_head_tail(const char *str)
{
        size_t siz;
        char *copy;
        ...
        if ((copy = malloc(siz)) == NULL)
            return(NULL);
        (void)memcpy(copy, str+1, siz-1);
        copy[siz-1] = '\0';
        return(copy);
}

/*parse crossbar config file*/
static int crossbar_config_parse(const char *file, struct userdata *u)
{
    struct json_object* obj_file;
    struct json_object* obj_block;
    struct json_object* obj_array;
    int i, narray, channels=0;
    struct crossbar_input* input;
    struct crossbar_output* output;
#ifdef HAVE_CONFIGURABLE_CROSSBAR_LIB
    char hw_variant[20] = {0};
    char *read_variant = NULL;
#endif

    obj_file = json_object_from_file(file);
    if (!obj_file) {
        pa_log_error("%s : read config file %s failed", __func__, file);
        return -1;
    }

    /*crossbar input*/
    u->crossbar_config_info.crossbar_input_channels = 0;
    u->crossbar_config_info.crossbar_input_count = 0;
    obj_block = json_object_object_get(obj_file, "crossbar-inputs");
    if (!obj_block) {
        pa_log_error("%s : json_object_object_get crossbar-input failed", __func__);
        return -1;
    }

    narray = json_object_array_length(obj_block);
    if(narray <= 0){
        pa_log_error("%s : no array found", __func__);
        return -1;
    }

    for(i=0; i < narray; i++) {
        input = pa_xnew0(struct crossbar_input, 1);
        if(input == NULL){
            pa_log_error("%s : memory alloc failed", __func__);
            return -1;
        }
        obj_array = json_object_array_get_idx(obj_block, i);
        input->name = strdup_remove_head_tail( json_object_to_json_string(json_object_object_get(obj_array, "name")));
        input->channels = json_object_get_int(json_object_object_get(obj_array, "channels"));
        input->offset = json_object_get_int(json_object_object_get(obj_array, "offset"));
        input->id =i;
        u->crossbar_config_info.crossbar_input_count++;
        channels += input->channels;
        PA_LLIST_PREPEND(struct crossbar_input, u->crossbar_config_info.crossbar_config_inputs, input);
    }

    if(channels > INPUT_CH_MAX){
        pa_log_error("%s : too many inputs %d", __func__, channels);
        return -1;
    } else {
        u->crossbar_config_info.crossbar_input_channels = channels;
    }

    /*crossbar output*/
    u->crossbar_config_info.crossbar_output_count = 0;
    u->crossbar_config_info.crossbar_output_channels = 0;
    obj_block = json_object_object_get(obj_file, "crossbar-output-devices");
    if (!obj_block) {
        pa_log_error("%s : json_object_object_get crossbar-outputs failed", __func__);
        return -1;
    }
    ...
    for(i=0; i< narray; i++) {
        output = pa_xnew0(struct crossbar_output, 1);
        if(output == NULL){
            pa_log_error("%s : memory alloc failed", __func__);
            return -1;
        }
        obj_array = json_object_array_get_idx(obj_block, i);
        output->name = strdup_remove_head_tail( json_object_to_json_string(json_object_object_get(obj_array, "name")));
        output->channels = json_object_get_int(json_object_object_get(obj_array, "channels"));
        output->offset = json_object_get_int(json_object_object_get(obj_array, "offset"));
        output->id = json_object_get_int(json_object_object_get(obj_array, "id"));
        output->external_trigger = json_object_get_int(json_object_object_get(obj_array, "external_trigger"));
        PA_LLIST_PREPEND(struct crossbar_output, u->crossbar_config_info.crossbar_config_outputs, output);
        u->crossbar_config_info.crossbar_output_count ++;
        u->crossbar_config_info.crossbar_output_channels+=output->channels;
    }

    u->crossbar_config_info.crossbar_libpath = NULL;

#ifdef HAVE_CONFIGURABLE_CROSSBAR_LIB
    /* get configurable crossbar lib */
    readEarlyPersistHalHelper(hw_variant);
    pa_log_debug("%s : read variant ID from EarlyPersist service is %s",__func__, hw_variant);
    obj_block = json_object_object_get(obj_file, "crossbar-libpath");
    if (!obj_block) {
        pa_log_error("%s : json_object_object_get crossbar-libpath failed", __func__);
        return -1;
    }

    narray = json_object_array_length(obj_block);
    if(narray <= 0){
        pa_log_error("%s : no array found for crossbar-libpath", __func__);
        return -1;
    }

    for(i=0; i< narray; i++) {
        obj_array = json_object_array_get_idx(obj_block, i);
        read_variant = strdup_remove_head_tail( json_object_to_json_string(json_object_object_get(obj_array, "hw_variant")));
        if (read_variant != NULL) {
            pa_log_debug("%s : read_variant is %s",__func__, read_variant);
            if (strcmp(read_variant, hw_variant) == 0) {    /* get the related hw variant lib path config */
                u->crossbar_config_info.crossbar_libpath = strdup_remove_head_tail( json_object_to_json_string_ext(json_object_object_get(obj_array, "path"),JSON_C_TO_STRING_NOSLASHESCAPE));
                pa_log_debug("%s : get configurable crossbar lib is %s",__func__, u->crossbar_config_info.crossbar_libpath);
                free(read_variant);
                break;
            }
            free(read_variant);
        } else
            pa_log_error("%s : no valid hw variant found!", __func__);
    }
#endif
    /* done*/
    return 0;
...
static void crossbar_config_free(struct userdata *u){
    struct crossbar_input *i;
    struct crossbar_output *o;

    PA_LLIST_FOREACH(i,  u->crossbar_config_info.crossbar_config_inputs) {
        PA_LLIST_REMOVE(struct crossbar_input, u->crossbar_config_info.crossbar_config_inputs, i);
        pa_xfree(i);
    }

    PA_LLIST_FOREACH(o,  u->crossbar_config_info.crossbar_config_outputs) {
        PA_LLIST_REMOVE(struct crossbar_output, u->crossbar_config_info.crossbar_config_outputs, o);
        pa_xfree(o);
    }

    if (u->crossbar_config_info.crossbar_libpath != NULL)
        free(u->crossbar_config_info.crossbar_libpath);

    return;
}


/*dump parsed config by log for debugging*/
static void crossbar_config_dump(struct userdata *u){

    struct crossbar_input *i;
    struct crossbar_output *o;

    pa_log_debug("%s : total %d channels input : ", __func__, u->crossbar_config_info.crossbar_input_channels );

    pa_log_debug("--- --------");
    PA_LLIST_FOREACH(i,  u->crossbar_config_info.crossbar_config_inputs)
        pa_log_debug("crossbar input: slot %02d, number of channels : %02d,  name: %s", i->offset, i->channels, i->name);

    pa_log_debug("--- --------");
    PA_LLIST_FOREACH(o,  u->crossbar_config_info.crossbar_config_outputs)
        pa_log_debug("crossbar out: id %02d, slot %02d, number of channels : %02d,  name: %s,external_trigger:%02d.", o->id, o->offset, o->channels, o->name, o->external_trigger);

}

static int crossbar_input_match(const pa_mix_info info, struct userdata *u, struct crossbar_input* i)
{
    const char *media_name = NULL;
    struct crossbar_input *input;

    media_name = ((pa_sink_input *)(info.userdata))->name;
    if(PA_UNLIKELY(media_name == NULL)) {
        return 0;
    }

    PA_LLIST_FOREACH(input, u->crossbar_config_info.crossbar_config_inputs){
        if(0 == strncmp(media_name, input->name,strnlen(input->name,63))){
             memcpy(i, input, sizeof(struct crossbar_input));
             return 1;
        }
    }

    //pa_log_warn("input source %s does not defined in crossbar config file, discarding", media_name);
    return 0;
}


static int crossbar_output_match(const struct output *x, struct userdata *u, struct crossbar_output* o)
{
    char *sink_name;
    struct crossbar_output *output;


    sink_name = x->sink_input->sink->name;

    if(sink_name == NULL) {
        pa_log_error("No available sink name of sink idx=%d", x->sink_input->sink->index);
        return 0;
    }

    PA_LLIST_FOREACH(output, u->crossbar_config_info.crossbar_config_outputs){
        if(pa_streq(sink_name, output->name)){
            memcpy(o, output, sizeof(struct crossbar_output));
            return 1;
        }
    }

    pa_log_warn("output sink %s does not defined in crossbar config file, ignoring", sink_name);
    return 0;
}
                    ...
                    const struct timeval *t, void *userdata)
{
    ...
    PA_LLIST_FOREACH(input, u->crossbar_config_info.crossbar_config_inputs){
        close(u->input_dump_fd[input->id]);
    }
    PA_LLIST_FOREACH(output, u->crossbar_config_info.crossbar_config_outputs){
         close(u->output_dump_fd[output->id]);
     }
    if(u->dump_buf)
    {
        pa_xfree(u->dump_buf);
    ...
    pa_log_debug("Pulseaudio Dump finished.");
}
...
static pa_dump_error_code_t dump_start(struct userdata *u, uint32_t seconds, const char *path)
{
    ...
    char file[256] = {0};
    struct crossbar_input *input = NULL;
    struct crossbar_output *output = NULL;
    ...
    PA_LLIST_FOREACH(input, u->crossbar_config_info.crossbar_config_inputs){
        sprintf(file, "%s/dump-input-%s-ch%02d-s32-48000Hz-%ds.raw", path,input->name, input->channels, seconds);
        if((u->input_dump_fd[input->id] = pa_open_cloexec(file, O_CREAT|O_RDWR, 0666)) < 0) {
            pa_log_error("create input dump file %s failed, ", file);
            return PA_DUMP_ERR_ACCESS;
        }
        max_channels = PA_MAX(max_channels, input->channels);
        pa_log_debug("created input dump file %s", file);
    }
    PA_LLIST_FOREACH(output, u->crossbar_config_info.crossbar_config_outputs){
        sprintf(file, "%s/dump-output-%s-ch%02d-s32-48000Hz-%ds.raw", path,output->name,output->channels,seconds);
        if((u->output_dump_fd[output->id] = pa_open_cloexec(file, O_CREAT|O_RDWR, 0666)) < 0) {
            pa_log_error("create output dump file %s failed, ", file);
            return PA_DUMP_ERR_ACCESS;
        }
        max_channels = PA_MAX(max_channels, output->channels);
        pa_log_debug("created output dump file %s", file);
    }

    u->dump_buf = pa_xmalloc0(FRAME_SIZE*4*max_channels);

    pa_core_rttime_new(u->core, pa_rtclock_now() + seconds*PA_USEC_PER_SEC, dump_complete_callback, u);
    pa_log_debug("Pulseaudio Dump start,dump path=%s", path);
    u->dumping = 1;
    return PA_DUMP_OK;
}
...
*/
static void paf_install(const void *src, void *dst[], unsigned src_channels,
                        unsigned src_samplesize, unsigned dst_channels, unsigned n)
{
    unsigned c;
    ...
    const int32_t *s32;
    int32_t *d;
    if(src_samplesize == sizeof(int32_t)){
        for (c = 0; c < dst_channels; c++) {
            s32 = (int32_t *) src + c;
            d = (int32_t *)dst[c];
            for (j = 0; j < n; j ++) {
                *d = (*s32)>>4;
                s32 = (int32_t*) s32 + src_channels;
                d = (int32_t*) d + 1;
            }
        }
    }
        ...
        for (c = 0; c < dst_channels; c++) {
            ...
            d = (int32_t *)dst[c];
            for (j = 0; j < n; j ++) {
                *d = (int32_t)(((int32_t) *(int16_t *)s16) << 12);
                s16 = (int16_t*) s16 + src_channels;
                d = (int32_t*) d + 1;
            }
        }
    }
...
static void paf_extract(const void *src[], unsigned src_channels, void *dst, unsigned dst_channels, unsigned dst_samplesize, unsigned n)
{
    ...
    if(dst_samplesize == sizeof(int32_t)){
        for (c = 0; c < src_channels; c++) {
            s = (int32_t *) src[c];
            d32 = (int32_t *) dst + c;
            for (j = 0; j < n; j ++) {
                *d32 = (*s)<<4;
                d32 = (int32_t*) d32 + dst_channels;
                s = (int32_t*) s + 1;
            }
        }
    }
...
static void APP_mix(pa_mix_info streams[], unsigned int nstreams, const pa_sample_spec *spec, void **zone, unsigned length, struct userdata *u) {
    int i,j,k,c;
    unsigned int samples, frames;
    struct crossbar_input cfg;
    struct crossbar_output o;
    ...
    struct output *x;
    size_t sink_index = 0;
    int input_ch_cnt = u->crossbar_config_info.crossbar_input_channels;
    int output_ch_cnt;
    unsigned channels = spec->channels;
    unsigned samplesize;
    uint8_t *d;
    uint8_t *src_u8;
    uint8_t *dst_u8;
    ...
    samplesize = (pa_sample_size(spec) <= sizeof(int16_t)) ? sizeof(int16_t) : sizeof(int32_t);
    samples = length/samplesize/channels;
    frames = samples/FRAME_SIZE;

    if(ncount++ % 5000 == 0)
        pa_log_debug("debug: current %d streams into crossbar %d,dumping state=%d.", nstreams,channels,u->dumping);

    if(samples%FRAME_SIZE != 0){
        pa_log_warn("warning: %d sample [%d bytes] ", samples, length);
    }

    /*clear each channel buffer is necessary*/
    for(i=0; i<input_ch_cnt; i++){
        memset(u->paf_prepro.data.sample[i], 0, samples * sizeof(PAF_AudioData));
    }

    /*install data to processing PAF struct */
    for(i=0; i<nstreams; i++) {
        if(crossbar_input_match(streams[i], u, &cfg)){
            src = streams[i].ptr;
            dst = (void *)&(u->paf_prepro.data.sample[cfg.offset]);
            src_channels = cfg.channels;
            paf_install(src, dst, channels, samplesize, src_channels, samples);
            ...
            if(u->dumping) {
                ...
                for(j=0; j<frames; j++) {
                    for(c=0;c<src_channels;c++)
                    {
                        d = u->dump_buf+c*samplesize;
                        for (k = 0; k< FRAME_SIZE; k++) {
                            memcpy(d, &src_u8[((j*FRAME_SIZE+k)*channels+c)*samplesize],samplesize);
                            d += samplesize*src_channels;
                        }
                    }
                    write(u->input_dump_fd[cfg.id], u->dump_buf,samplesize*src_channels*FRAME_SIZE);
                }
                pa_mutex_unlock(u->mutex);
            }
        }
    ...
    for(i=0; i<frames; i++) {
        ...
        output_ch_cnt = 0;
        /*cpoy processed channels data to each sinks*/
        PA_LLIST_FOREACH(x, u->thread_info.active_outputs){
            if(crossbar_output_match(x, u, &o)) {
                src_offset = o.offset;
                src_channels = o.channels;
                sink_index = o.id;
                output_ch_cnt+=src_channels;
            }

            if(ncount % 5000 == 0)
                pa_log_debug("sink name %s , sink_index %d", o.name, o.id);

            src = (void *)&(u->paf_prepro.data.sample[src_offset]);
              if(samplesize == sizeof(int32_t))
                dst = (int32_t *)zone[sink_index] + FRAME_SIZE * i * channels;
            else
                dst = (int16_t *)zone[sink_index] + FRAME_SIZE * i * channels;
            paf_extract(src, src_channels, dst, channels, samplesize, FRAME_SIZE);
            if(u->dumping) {
                pa_mutex_lock(u->mutex);
                dst_u8 =(uint8_t *)dst;
                for(c=0;c<src_channels;c++)
                {
                    d = u->dump_buf+c*samplesize;
                    for (k = 0; k< FRAME_SIZE; k++) {
                        memcpy(d, &dst_u8[(k*channels+c)*samplesize],samplesize);
                        d += samplesize*src_channels;
                    }
                }
                write(u->output_dump_fd[sink_index], u->dump_buf,samplesize*src_channels*FRAME_SIZE);
                pa_mutex_unlock(u->mutex);
            }
        }
    }
...
static size_t sink_streams_APP(
    ...
    APP_mix(streams, nstreams, spec, data, length, u);
...
static void pa_app_sink_render(struct userdata *u, size_t length, pa_memchunk *result) {
    ...
    if (s->thread_info.state == PA_SINK_SUSPENDED) {
        PA_LLIST_FOREACH(j, u->thread_info.active_outputs) {
            if(crossbar_output_match(j, u, &o))
                sink_index = o.id;
    ...
    if (n == 0) {
        ...
        void *ptr[u->crossbar_config_info.crossbar_output_count];
        PA_LLIST_FOREACH(j, u->thread_info.active_outputs){
            if(crossbar_output_match(j, u, &o))
                sink_index = o.id;
...
static void render_memblock(struct userdata *u, struct output *o, size_t length) {
    ...
    int i;
    int ret;
    struct crossbar_output co;

    pa_assert(u);
    pa_assert(o);

    //init co in default value
    ...
    while (!pa_memblockq_is_readable(o->memblockq)) {
        struct output *j;
        pa_memchunk zone_chunk[u->crossbar_config_info.crossbar_output_count];

        /*initialize*/
        for(i=0;i<u->crossbar_config_info.crossbar_output_count;i++) {
                pa_memchunk_reset(&zone_chunk[i]);
        }

        /* Render data! */
        pa_app_sink_render(u, length, &zone_chunk[0]);
        u->thread_info.counter += zone_chunk[0].length;

        /* OK, let's send this data to the other threads */
        PA_LLIST_FOREACH(j, u->thread_info.active_outputs) {
            crossbar_output_match(j, u, &co);
            id = co.id;
            if (j == o){
                continue;
            }
            pa_asyncmsgq_post(j->inq, PA_MSGOBJECT(j->sink_input), SINK_INPUT_MESSAGE_POST,
                    NULL, 0, &zone_chunk[id], NULL);
        }
        /* And place it directly into the requesting output's queue */
        crossbar_output_match(o, u, &co);
        id = co.id;
    ...
    switch (code) {
        ...
        case SINK_INPUT_MESSAGE_POST:
            ...
            if (PA_SINK_IS_OPENED(o->sink_input->sink->thread_info.state))
            {
                ...
                if( ret < 0 ){
                    pa_log_warn("---------------------------- WARNING : PULSEAUDIO BUFFER QUEUE OVERFLOW : POSSIBLE AUDIO DROPS ---------------------------- ");
                    pa_log_warn("%s(): pa_memblockq_push_align failed %d", __func__, ret);
                    pa_log_warn("%s(): buffer has full? fill level %0.2fms/%0.2fms, drop all buffered data!", __func__,
                ...
                {
                        /*enable the external_trigger should add <"external_trigger" : 1> in crossbar.conf relate output sink file*/
                        if(crossbar_output_match(o,o->userdata, &co))
                        {
                                if(co.external_trigger)
                                {
                                        pa_asyncmsgq_post(o->sink_input->sink->asyncmsgq, PA_MSGOBJECT(o->sink_input->sink), PA_SINK_MESSAGE_ADD_DATA, NULL, 0, NULL, NULL);
                                }
                        }
                }
                pa_memblock_unref(chunk->memblock);
...
static int output_create_sink_input(struct output *o) {
    ...
    data.flags = PA_SINK_INPUT_DONT_MOVE|PA_SINK_INPUT_NO_CREATE_ON_SUSPEND;
...
static struct output *output_new(struct userdata *u, pa_sink *sink) {
    ...
    pa_log_debug("new output memblockq size %zd", buf_length);
...
static pa_hook_result_t sink_put_hook_cb(pa_core *c, pa_sink *s, struct userdata* u) {
    ...
    if (u->automatic) {
        ...
        /* Check if the sink is defined in crossbar config file */
            PA_LLIST_FOREACH(co, u->crossbar_config_info.crossbar_config_outputs){
                        if(pa_streq(s->name, co->name)){
                                    pa_log_debug("sink name %s match with crossbar config", s->name);
                                    match = true ;
                                    break;
                        }
            }
            if(!match){
                        pa_log_debug("new sink name %s does not match with any sink in crossbar config", s->name);
                        return PA_HOOK_OK;
            }
    }
...
static int PAF_AudioFrame_init(PAF_AudioFrame *paf, int channels, int nbytes_pre_channel)
{
    int i;

    paf->sampleCount = FRAME_SIZE;
    paf->data.nChannels = channels;
    paf->data.sample = (PAF_AudioData **)pa_xmalloc0(paf->data.nChannels * sizeof(PAF_AudioData *));
    for(i=0; i<channels ;i++) {
        paf->data.sample[i] = (PAF_AudioData *)pa_xmalloc0(nbytes_pre_channel);
    }
    return 0;
}

static int PAF_AudioFrame_free(PAF_AudioFrame *paf, int channels)
{
    int i;
...
{
    if (u->crossbar_config_info.crossbar_libpath != NULL)
        u->pDlopen_handle = dlopen(u->crossbar_config_info.crossbar_libpath, RTLD_NOW);
    else
        u->pDlopen_handle = dlopen(AUDIOPROCESSING_LIBPATH, RTLD_NOW);
    if(u->pDlopen_handle == NULL){
        pa_log("dlopen %s failed", u->crossbar_config_info.crossbar_libpath != NULL ? u->crossbar_config_info.crossbar_libpath : AUDIOPROCESSING_LIBPATH);
        return -1;
    }

    pa_log_debug("dlopen %s success", u->crossbar_config_info.crossbar_libpath != NULL ? u->crossbar_config_info.crossbar_libpath : AUDIOPROCESSING_LIBPATH);
    ...
    if(!u->ap_create || !u->ap_apply || !u->ap_msg_write || !u->ap_msg_read){
        pa_log("dlsym %s failed", u->crossbar_config_info.crossbar_libpath != NULL ? u->crossbar_config_info.crossbar_libpath : AUDIOPROCESSING_LIBPATH);
        return -2;
    }
...
int pa__init(pa_module*m) {
    ...
    int resample_method = PA_RESAMPLER_COPY;;
    pa_sample_spec ss;
    pa_channel_map map;
    struct output *o;
    uint32_t idx;
    ...
    pa_log_debug("combine sink initiation");
    ...
    pa_log_debug("crossbar config file parsing...");
    if(crossbar_config_parse(PA_CROSSBAR_CONFIG_FILE, u) < 0) {
        pa_log_error("Failed to parse crossbar config file : %s", PA_CROSSBAR_CONFIG_FILE);
        goto fail;
    }
    crossbar_config_dump(u);
    ...
    if (!u->automatic) {
        ...
        while ((n = pa_split(slaves, ",", &split_state))) {
            ...
            max_request = pa_sink_get_max_request(slave_sink);
            if(max_request > max_max_request)
                max_max_request = max_request;
        }
    ...
    PAF_AudioFrame_init(&(u->paf_prepro), u->crossbar_config_info.crossbar_input_channels,
        max_max_request/(ss.channels)/sizeof(int16_t)*sizeof(int32_t));
...
void pa__done(pa_module*m) {
    ...
    PAF_AudioFrame_free(&(u->paf_prepro), u->crossbar_config_info.crossbar_input_channels);
    audio_processing_lib_close(u);

    crossbar_config_free(u);
