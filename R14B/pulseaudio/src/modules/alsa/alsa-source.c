...
static int mmap_read(struct userdata *u, pa_usec_t *sleep_usec, bool polled, bool on_timeout) {
    ...
    for (;;) {
        ...
        if (PA_UNLIKELY(n_bytes <= 0)) {
            ...
            if (polled)
                PA_ONCE_BEGIN {
                    pa_log_debug("POLLIN event is delay, if happen overrun, please check delay whether too long!");
                } PA_ONCE_END;
...
static int unsuspend(struct userdata *u, bool recovering) {
    ...
    if ((err = pa_alsa_robust_open(&u->pcm_handle, u->device_name, SND_PCM_STREAM_CAPTURE,
                            SND_PCM_NONBLOCK|
...
static void thread_func(void *userdata) {
    ...
    for(;;) {
        ...
        for (;;) {
            ...
            if ((PA_SOURCE_IS_OPENED(u->source->thread_info.state)) && (snd_pcm_state(u->pcm_handle) != SND_PCM_STATE_SUSPENDED)) {
                int work_done;
                ...
                if (u->first) {
                    ...
                    if ((ret = pa_alsa_robust_start(u->pcm_handle, ALSA_API_FAIL_RETRY_CNT)) < 0) {
                        pa_log_warn("[alsa-source]pa_alsa_robust_start failed %d", ret);
                        goto fail;
                    }
    ...
    /*                 pa_log_debug("Waking up in %0.2fms (system clock).", (double) cusec / PA_USEC_PER_MSEC); */
            ...
            if(time_count >= LOG_INTERVAL * u->source->sample_spec.rate/(u->fragment_size/u->frame_size)) {
                pa_log_info("Alsa Source %s thread is running!", u->source->name);
                time_count = 0;
            }
#endif
        }
    ...
    fail:
        ...
        pa_log_warn("Never shutdown alsa-source thread! Try to restart the PCM...");
        do {
            pa_log_warn("[Exception Recovery]close_pcm start");
            close_pcm(u);
            pa_log_warn("[Exception Recovery]close_pcm done");
...
pa_source *pa_alsa_source_new(pa_module *m, pa_modargs *ma, const char*driver, pa_card *card, pa_alsa_mapping *mapping) {
    ...
    if (mapping && mapping->description && mapping->name)
