...
/* #define DEBUG_TIMING */
#define DEBUG_POLLING_TIMING
/* #define DEBUG_TIMING */
#define DEBUG_MSG_PERIODIC
...
#ifdef DEBUG_MSG_PERIODIC
#define LOG_INTERVAL (30) /* 30s, thread preodic debug log every 30s */
#endif
...
static int try_recover(struct userdata *u, const char *call, int err) {
    ...
    if ((err = pa_alsa_robust_recover(u->pcm_handle, err, 1, ALSA_API_FAIL_RETRY_CNT)) < 0) {
        ...
        do {
            pa_log_warn("[Exception Recovery]close_pcm start");
            close_pcm(u);
            pa_log_warn("[Exception Recovery]close_pcm done");
            pa_msleep(ALSA_API_FAIL_RETRY_TIME);
            pa_log_warn("[Exception Recovery]reopen pcm start");
            ret = unsuspend(u, true);
            pa_log_warn("[Exception Recovery]reopen pcm done");
        }while(ret < 0);
...
static int mmap_write(struct userdata *u, pa_usec_t *sleep_usec, bool polled, bool on_timeout) {
    ...
    size_t left_to_play = 0, input_underrun = 0;
    unsigned j = 0;
    ...
    for (;;) {
        ...
        if (PA_UNLIKELY((n = pa_alsa_safe_avail(u->pcm_handle, u->hwbuf_size, &u->sink->sample_spec)) < 0)) {
            ...
            /* ALSA device suspended */
            if(((int) n == -EBADFD) && (snd_pcm_state(u->pcm_handle) == SND_PCM_STATE_SUSPENDED))
                break;
        ...
        if (PA_UNLIKELY(n_bytes <= u->hwbuf_unused)) {
            ...
            if (polled)
                PA_ONCE_BEGIN {
                    pa_log_debug("POLLOUT event is delay, if happen underrun, please check delay whether too long!");
                } PA_ONCE_END;
...
/*             pa_log_debug("%lu frames to write", (unsigned long) frames); */
            ...
            if (PA_UNLIKELY((err = pa_alsa_safe_mmap_begin(u->pcm_handle, &areas, &offset, &frames, u->hwbuf_size, &u->sink->sample_spec)) < 0)) {
                ...
                /* ALSA device suspended */
                if((err == -EBADFD) && (snd_pcm_state(u->pcm_handle) == SND_PCM_STATE_SUSPENDED))
                    break;
            ...
            if (PA_UNLIKELY((sframes = snd_pcm_mmap_commit(u->pcm_handle, offset, frames)) < 0)) {
                ...
                /* ALSA device suspended */
                if(((int) sframes == -EBADFD) && (snd_pcm_state(u->pcm_handle) == SND_PCM_STATE_SUSPENDED))
                    break;
...
static int suspend(struct userdata *u) {
    ...
    if (!u->pcm_handle) {
        pa_log_info("pcm_handle is NULL");
        return -1;
    }
    ...
    if (u->always_open) {
        ...
        pa_log_debug("alsa sink suspend, but remain ALSA device to be opened!");
    } else {
        /* Close PCM device */
        close_pcm(u);
    }
...
static int unsuspend(struct userdata *u, bool recovering) {
    ...
    /* if ALSA device not closed, restart it again in prepare state */
    if (u->always_open && u->pcm_handle != NULL) {
        if (snd_pcm_state(u->pcm_handle) == SND_PCM_STATE_PREPARED) {
            pa_log_debug("alsa sink: resume from prepare state...");
            goto restart;
        }
    }

    if ((err = pa_alsa_robust_open(&u->pcm_handle, device_name ? device_name : u->device_name, SND_PCM_STREAM_PLAYBACK,
                            SND_PCM_NONBLOCK|
                            SND_PCM_NO_AUTO_RESAMPLE|
                            SND_PCM_NO_AUTO_CHANNELS|
                            SND_PCM_NO_AUTO_FORMAT,
                            ALSA_API_FAIL_RETRY_CNT)) < 0) {
        pa_log("Error opening PCM device %s: %s", u->device_name, pa_alsa_strerror(err));
        pulseaudio_list_directory("/dev/snd");
        goto fail;
    }
...
static void sink_set_volume_cb(pa_sink *s) {
    ...
    if (u->mixer_path->has_dB) {
        ...
        //init new_soft_volume in default value
        new_soft_volume.channels = PA_CHANNELS_MAX;
        for (int i = 0; i < PA_CHANNELS_MAX; i++)
            new_soft_volume.values[i] = PA_VOLUME_MAX;
...
static int process_rewind(struct userdata *u) {
    ...
    rewind_nbytes = rewind_nbytes/64*64;  //COC media modification
...
static void thread_func(void *userdata) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if ((PA_SINK_IS_OPENED(u->sink->thread_info.state)) && (snd_pcm_state(u->pcm_handle) != SND_PCM_STATE_SUSPENDED)) {
                int work_done;
    ...
    /*             pa_log_debug("work_done = %i", work_done); */
                ...
                if (work_done) {
                    ...
                    if (u->first) {
                        ...
                        if ((ret = pa_alsa_robust_start(u->pcm_handle, ALSA_API_FAIL_RETRY_CNT)) < 0) {
                            pa_log_warn("[alsa-sink]pa_alsa_robust_start failed %d", ret);
                            goto fail;
                        }
...
#ifdef DEBUG_POLLING_TIMING
            /*Print debug meaasge*/
            pa_usec_t my_sleep, my_real_sleep;
            snd_pcm_uframes_t period_size, buffer_size;
            period_size = u->fragment_size / u->frame_size;
            buffer_size = u->hwbuf_size / u->frame_size;

            my_sleep = pa_rtclock_now();
#endif
...
#ifdef DEBUG_POLLING_TIMING
            my_real_sleep = pa_rtclock_now() - my_sleep;
            if(my_real_sleep > PA_USEC_PER_SEC * (buffer_size)/(u->sink->sample_spec.rate)){
                /*if poll sleep almost reach whole buffer size, underrun might happen, print a log here for debug */
                pa_log_debug("debug: poll woke us up after: %d us", my_real_sleep);
            }
#endif
...
#ifdef DEBUG_MSG_PERIODIC
            /* Output periodic log to identify thread is running */
            if (revents & POLLOUT) {
                time_count++;
            }

           if(time_count >= LOG_INTERVAL * u->sink->sample_spec.rate/(u->fragment_size/u->frame_size)) {
                pa_log_info("Alsa sink %s thread is running!", u->sink->name);
                time_count = 0;
           }
#endif
        }
    ...
    fail:
        ...
        pa_log_warn("Never shutdown alsa-sink thread! Try to restart the PCM...");
        do {
            pa_log_warn("[Exception Recovery]close_pcm start");
            close_pcm(u);
            pa_log_warn("[Exception Recovery]close_pcm done");
            pa_msleep(ALSA_API_FAIL_RETRY_TIME);
            pa_log_warn("[Exception Recovery]reopen pcm start");
            recovery_ret = unsuspend(u, true);
            pa_log_warn("[Exception Recovery]reopen pcm done");
        }while(recovery_ret < 0);
    }
finish:
    pa_log_error("Thread shutting down");
}
...
pa_sink *pa_alsa_sink_new(pa_module *m, pa_modargs *ma, const char*driver, pa_card *card, pa_alsa_mapping *mapping) {
    ...
    size_t frame_size;
    bool use_mmap = true, b, use_tsched = true, d, ignore_dB = false, namereg_fail = false, deferred_volume = false, set_formats = false, fixed_latency_range = false, always_open = false;
    ...
    if (pa_modargs_get_value_boolean(ma, "always_open", &always_open) < 0) {
        pa_log("Failed to parse always_open argument.");
        goto fail;
    }
