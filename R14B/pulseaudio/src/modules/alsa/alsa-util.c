...
void pulseaudio_list_directory(const char *pathname) {
    DIR *dir;
    struct dirent *e;
    pa_assert(pathname);

    pa_log_info("list files under %s : ", pathname);

    /* open this as directory */
    if (!(dir = opendir(pathname))) {
        pa_log_error("can not open directory %s", pathname);
        return;
    }
    /* read files under directory*/
    while ((e = readdir(dir)))
        pa_log_info("file : %s", e->d_name);
...
int pa_alsa_set_sw_params(snd_pcm_t *pcm, snd_pcm_uframes_t avail_min, bool period_event) {
    ...
    if ((err = snd_pcm_sw_params_set_stop_threshold(pcm, swparams, buffer_size > 0 ? buffer_size : boundary)) < 0) {
        pa_log_warn("Unable to set stop threshold: %s\n", pa_alsa_strerror(err));
        return err;
    }

    if ((err = snd_pcm_sw_params_set_start_threshold(pcm, swparams, buffer_size > 0 ? buffer_size : (snd_pcm_uframes_t) -1)) < 0) {
...
snd_pcm_t *pa_alsa_open_by_device_string(
    ...
    for (;;) {
        ...
        if ((err = snd_pcm_open(&pcm_handle, d, mode,
            ...
            retry_cnt++;
            if(retry_cnt >PULSEAUDIO_ALSA_OPEN_FAIL_RETRY_CNT)
            {
                pa_log_error("Error opening PCM device %s: %s", d, pa_alsa_strerror(err));
                pulseaudio_list_directory("/dev/snd");
                goto fail;
            }
            else
            {
                pa_log_error("Error opening PCM device %s: %s,retrying...", d, pa_alsa_strerror(err));
                usleep(PULSEAUDIO_ALSA_OPEN_FAIL_RETRY_TIME);
                continue;
            }
        }
...
int pa_alsa_recover_from_poll(snd_pcm_t *pcm, int revents) {
    ...
    switch (state) {
        ...
        case SND_PCM_STATE_XRUN:
            if ((err = pa_alsa_robust_recover(pcm, -EPIPE, 1, ALSA_API_FAIL_RETRY_CNT)) != 0) {
...
snd_pcm_sframes_t pa_alsa_safe_avail(snd_pcm_t *pcm, size_t hwbuf_size, const pa_sample_spec *ss) {
    ...
    if (PA_UNLIKELY(k >= hwbuf_size * 5 ||
        ...
        PA_ONCE_BEGIN {
            pa_log_debug("available buffer is exceptionally large: %lu bytes. POLL event is delayed",
                   (unsigned long) k);
...
int pa_alsa_robust_open(snd_pcm_t **pcmp, const char *device_name, snd_pcm_stream_t stream, int mode, unsigned int retry_times) {
    pa_assert(pcmp);
    ...
    for(i = 0; i < retry_times; i++) {
        if ((ret = snd_pcm_open(pcmp, device_name, stream, mode)) < 0) {
            pa_log_warn("Error opening PCM device %s: %s,retrying...", device_name, pa_alsa_strerror(ret));
...
int pa_alsa_robust_start(snd_pcm_t *pcm, unsigned int retry_times) {
    pa_assert(pcm);
    ...
    for(i = 0; i < retry_times; i++) {
        if ((ret = snd_pcm_start(pcm)) < 0) {
            pa_log_warn("snd_pcm_start failed with %d, retrying...", ret);
            pa_msleep(ALSA_API_FAIL_RETRY_TIME);
        } else {
            break;
        }
...
int pa_alsa_robust_recover(snd_pcm_t *pcm, int err, int silent, unsigned int retry_times) {
    pa_assert(pcm);
    ...
    for(i = 0; i < retry_times; i++) {
        if ((ret = snd_pcm_recover(pcm, err, silent)) < 0) {
            pa_log_warn("snd_pcm_recover failed with %d, retrying...", ret);
            pa_msleep(ALSA_API_FAIL_RETRY_TIME);
        } else {
            break;
        }
