...
int main(int argc, char *argv[]) {
...
#if 0//defined(__linux__) && defined(__OPTIMIZE__)
    /*
    ...
    */
    ...
    /* log level test */
    pa_log_error("pa_log_error show\n");
    pa_log_warn("pa_log_warn show\n");
    pa_log_notice("pa_log_notice show\n");
    pa_log_info("pa_log_info show\n");
    pa_log_debug("pa_log_debug show\n");
    pa_log("pa_log show\n");
    ...
    pa_signal_new(SIGABRT, signal_callback, c);
    pa_signal_new(SIGFPE, signal_callback, c);
    pa_signal_new(SIGILL, signal_callback, c);
#ifndef __FreeBSD_kernel__
    pa_signal_new(SIGSEGV, signal_callback, c);
#endif
#ifdef SIGSYS
    pa_signal_new(SIGSYS, signal_callback, c);
#endif
#endif
	...
	if(c->realtime_scheduling)
		pa_make_realtime(c->realtime_priority);
