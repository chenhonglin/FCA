...
/* Module Parameter to disable RSDB mode as to
 * support P2P GC which works only in MIMO mode 
 */
int rsdb_mode_cfg = -1;
module_param(rsdb_mode_cfg, int, 0);
#endif
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if(-1 != rsdb_mode_cfg)
	{
		if (FW_SUPPORTED(dhd, rsdb))
		{
			...
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR,
					iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) {
					DHD_ERROR(("%s rsdb_mode is failed to set ret= %d\n", __FUNCTION__, ret));
			}
		}
	}
