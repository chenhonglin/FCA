...
#define BXT_INIT_TIMEOUT	3000
#define BXT_ROM_INIT_TIMEOUT	1000
...
#ifdef CONFIG_SND_SOC_BXT_FW_STATUS_REPORT
static char *fw_load_status_table[FW_LOAD_STATUS_MAX] = {
	[FW_LOAD_INITING] = "initing",
	[FW_LOAD_SUCCESS] = "success",
	[FW_LOAD_FAIL] = "fail",
};
static int fw_load_cur_status;

void bxt_set_cur_fw_load_status(int status)
{
	if (status < FW_LOAD_INITING || status >= FW_LOAD_STATUS_MAX) {
		pr_err("%s : Invalid fw load status value , set fail !\n", __func__);
		return;
	}
	fw_load_cur_status = status;
	pr_debug("%s : Set fw load status to %s !\n", __func__, fw_load_status_table[fw_load_cur_status]);
}
EXPORT_SYMBOL_GPL(bxt_set_cur_fw_load_status);

static ssize_t fw_load_status_show(struct class *class, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", fw_load_status_table[fw_load_cur_status]);
}
CLASS_ATTR(fw_load_status, 0644, fw_load_status_show, NULL);

int bxt_fw_status_create_sysfs(struct skl *skl)
{
	skl->fw_status_sysfs = class_create(THIS_MODULE, "hifi_fw");
	if (skl->fw_status_sysfs == NULL) {
		pr_err("%s : Create fw class file failed!\n", __func__);
	} else {
		if (class_create_file(skl->fw_status_sysfs, &class_attr_fw_load_status)) {
			class_destroy(skl->fw_status_sysfs);
			skl->fw_status_sysfs = NULL;
			pr_err("%s : Add fw class attr failed!\n", __func__);
		} else {
			pr_debug("%s : Added fw class attr : fw_load_status!\n", __func__);
		}
	}
	return 0;
}
EXPORT_SYMBOL_GPL(bxt_fw_status_create_sysfs);

void bxt_fw_status_remove_sysfs(struct skl *skl)
{
	if (skl->fw_status_sysfs) {
		class_remove_file(skl->fw_status_sysfs, &class_attr_fw_load_status);
		class_destroy(skl->fw_status_sysfs);
		skl->fw_status_sysfs = NULL;
	}
}
EXPORT_SYMBOL_GPL(bxt_fw_status_remove_sysfs);
#endif
...
#ifdef CONFIG_SND_SOC_BXT_FW_STATUS_REPORT
		bxt_set_cur_fw_load_status(FW_LOAD_FAIL);
#endif
		return ret;
	}
...
#ifdef CONFIG_SND_SOC_BXT_FW_STATUS_REPORT
	bxt_set_cur_fw_load_status(FW_LOAD_SUCCESS);
#endif
	return 0;
}
