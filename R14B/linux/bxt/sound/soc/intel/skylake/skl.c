...
#ifdef CONFIG_SND_SOC_FCA_ALSA
bool get_anc_ipc_sync(void) {
    return anc_ipc_sync;
}
EXPORT_SYMBOL_GPL(get_anc_ipc_sync);
#endif
...
static int skl_find_mchine(struct skl *skl, void *driver_data)
{
...
#ifdef CONFIG_BJEVN60_VIRTUAL
	struct sst_acpi_mach *machines = mach;
#endif
	struct hdac_bus *bus = ebus_to_hbus(&skl->ebus);

	/*
	 * BJEV Hardware is not configuring Audio Codec from SOC, removed
	 * codec information from this driver
	 */
#ifndef CONFIG_BJEVN60_VIRTUAL
	if ((skl->pci->device == 0x9df0) || (skl->pci->device == 0x9dc8)
		...
		goto out;
#else
	if ((skl->pci->device == 0x5a98)) {
		for (mach = machines; mach->id[0]; mach++) {
			if ((strncmp((char *)(mach->id), "INT34C3",7)) == 0) {
				goto out;
			}
		}
	}
...
static void skl_probe_work(struct work_struct *work)
{
	...
	if (!(skl->pci->device == 0x9df0 ||
	      skl->pci->device == 0x34c8 || skl->pci->device == 0x24f0)) {
#else
	if (!(skl->pci->device == 0x9df0 || skl->pci->device == 0x5a98 ||
              skl->pci->device == 0x34c8 || skl->pci->device == 0x24f0)) {
#endif
...
struct sst_generic_ipc* ipc_handle;
EXPORT_SYMBOL_GPL(ipc_handle);
		     ...
		     const struct pci_device_id *pci_id)
{
	...
	bxt_fw_status_create_sysfs(skl);
#endif
...
#ifdef CONFIG_SND_SOC_FCA_ALSA
	/*Enable anc ipc after cold boot*/
	anc_ipc_sync = true;
#endif
	return 0;
...
static void skl_remove(struct pci_dev *pci)
{
	...
	skl_nhlt_free(skl->nhlt);
#ifdef CONFIG_SND_SOC_BXT_FW_STATUS_REPORT
	bxt_fw_status_remove_sysfs(skl);
#endif
	skl_free(ebus);
