/*
 * drivers/video/ds2_display.c
 * FPDLink Serializer/Deserializer for ds2 display driver
 */
...
extern int ser_read_isr(void)
{
	...
	dev_dbg(&global_client->dev, "INT reciever 0xC7: 0x%u\n", val);
	return err;
}
...
static void register_touch_controller(struct work_struct *work_arg)
{
	...
	/* Then the ds2_display_rec pointer */
	struct ds2_display_rec *data =
		container_of(delayed_work, struct ds2_display_rec, work_item);

	pr_notice("ds2_display: delay complete - starting touch driver\n");
...
static void check_lock_status(struct work_struct *work_arg)
{
	...
	if (err == 1) {
		pr_notice("ds2_display: LVDS_LOCK detected, starting delay\n");
...
static int ds2_display_enable(struct i2c_client *client)
{
	struct ds2_display_rec *data = i2c_get_clientdata(client);
	int err;
	uint8_t val;
	...
	do {
		...
		/* Create work queue */
		data->work_queue = create_singlethread_workqueue("ds2_display");
	...
	if (err < 0) {
		pr_err("ds2_display_enable: i2c read/write errir!\n");
		return err;
	} else {
		if (data->touch_client) {
			pr_notice("ds2_display:Releasing touch client\n");
			i2c_release_client(data->touch_client);
			data->touch_client = 0;
		}
		pr_notice("ds2_display:Report success\n");
		return 0;
	}
}

static void ds2_display_disable(struct i2c_client *client)
{
	struct ds2_display_rec *data = i2c_get_clientdata(client);
...
static release_all_resources(struct ds2_display_rec *data)
{
	if (data->work_queue) {
...
static int ds2_display_probe(struct i2c_client *client,
				const struct i2c_device_id *id)
{
	...
	int err;
	pr_notice("ds2_display_probe: success\n");
...
static int ds2_display_remove(struct i2c_client *client)
{
	struct ds2_display_rec *data = i2c_get_clientdata(client);
	ds2_display_disable(client);
	i2c_set_clientdata(client, NULL);
	return 0;
}

#ifdef CONFIG_PM_SLEEP
static int ds2_display_suspend(struct device *dev)
{
...
static int ds2_display_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	return ds2_display_enable(client);
}
#endif

static const struct i2c_device_id ds2_display_id[] = {
	{ "gw_display", 0 },
	{ }
};

MODULE_DEVICE_TABLE(i2c, ds2_display_id);

#ifdef CONFIG_PM_SLEEP
static SIMPLE_DEV_PM_OPS(ds2_display_pm_ops, ds2_display_suspend, ds2_display_resume);
#endif

static struct i2c_driver ds2_display_driver = {
	.driver = {
		.name  = "ds2_display",
		.owner = THIS_MODULE,
#ifdef CONFIG_PM_SLEEP
		.pm	   = &ds2_display_pm_ops,
#endif
	},
	.probe    = ds2_display_probe,
	.remove   = ds2_display_remove,
	.id_table = ds2_display_id,
};

module_i2c_driver(ds2_display_driver);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("ds2_display FPDLink Serializer driver");
MODULE_ALIAS("i2c:ds2_display");
