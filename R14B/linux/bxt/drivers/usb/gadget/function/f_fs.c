...
struct ffs_ep {
	...
	int				timeout; /* timeout in milli-seconds */
};
...
static ssize_t ffs_epfile_io(struct file *file, struct ffs_io_data *io_data)
{
	...
	if (epfile->ep != ep) {
		...
		if (epfile->ep != ep)
			...
			ret = 0; /* return 0 when it is timed out */
			     ...
			     unsigned long value)
{
	...
	if (likely(epfile->ep)) {
		...
		case FUNCTIONFS_ENDPOINT_SET_TIMEOUT:
			ret = __get_user(timeout, (int __user *)value);
			if (ret == 0)
				epfile->ep->timeout = timeout;
			break;
		case FUNCTIONFS_ENDPOINT_GET_TIMEOUT:
			ret = __put_user(epfile->ep->timeout,
						(int __user *)value);
			break;
		default:
			ret = -ENOTTY;
