	...
	do {
		...
		if (pmadapter->pending_ioctl) {
			...
			pmadapter->pending_ioctl = MFALSE;
                ...
                /** SDIO wakeup fail handle */
                if (pmadapter->wakeup_timeout_flag) {
                    pcb->moal_read_reg(pmadapter->pmoal_handle, 5, &val);
                    /** poll INT to avoid INT missing */
                    if (val & MBIT(1))
                        wlan_interrupt(pmadapter);
                    else
                        wlan_pm_wakeup_card(pmadapter, MTRUE);
                    /** trigger dump when wakeup fail timeout */
                    if (pmadapter->ps_state != PS_STATE_AWAKE
                       && pmadapter->wakeup_timeout_count > 2)
                       wlan_recv_event(wlan_get_priv
                           (pmadapter,
                           MLAN_BSS_ROLE_ANY),
                           MLAN_EVENT_ID_DRV_DBG_DUMP,
                           MNULL);
                    pmadapter->wakeup_timeout_flag = MFALSE;
                    if (pmadapter->ps_state == PS_STATE_AWAKE)
                        pmadapter->wakeup_timeout_count = 0;
                }
