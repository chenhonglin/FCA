...
// vendor id of TBM module
#define TBM_VID                 0x2BE9
/*-------------------------------------------------------------------------*/
...
usbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
  ...
  /* this patch will be removed once tbm over ethernet is introduced. We are checking here for TBM Vendor id. */
   if ( (xdev->descriptor.idVendor == TBM_VID) && (USB_CLASS_COMM == interface->desc.bInterfaceClass) &&
        (USB_CDC_SUBCLASS_NCM == interface->desc.bInterfaceSubClass) )
   {
      strcpy (net->name, "ncm%d");
   }
   else if ( (xdev->descriptor.idVendor == TBM_VID) && (USB_CLASS_COMM == interface->desc.bInterfaceClass) &&
             (USB_CDC_SUBCLASS_ETHERNET == interface->desc.bInterfaceSubClass) )
   {
      strcpy (net->name, "ecm%d");
   }
   else
   {
