...
//SWPF
#define CY_CORE_DEVICE_RESET_BY_PLATFORM_INIT	true

extern unsigned int event_path_global;
unsigned int cy_native_x_pos;
unsigned int cy_native_y_pos;
unsigned int cy_native_z_pos;
...
static int cyttsp6_hw_reset_(struct cyttsp6_core_data *cd)
{
	...
	if (rc == -ENOSYS){
		dev_err(cd->dev, "%s: soft reset call\n", __func__);
...
static int cyttsp6_request_reset_(struct device *dev)
{
	...
	if (rc < 0)
		dev_err(dev, "%s: Error on h/w reset r=%d mode = %d\n", __func__, rc, cd->mode);
...
reset:
	...
	if (reset_needed || retry != CY_CORE_STARTUP_RETRY_COUNT) {
		...
		rc = cyttsp6_reset_and_wait(cd);
		if (rc < 0) {
			dev_err(cd->dev, "%s: Error on h/w reset r=%d mode = \n",
					__func__, rc, cd->mode);
			if (retry--)
				goto reset;
			goto exit;
...
exit_no_wd:
	...
	/* Required for signal to the TTHE */
	dev_info(cd->dev, "%s: cyttsp6_exit startup rc=%d... mode = %d\n", __func__, rc, cd->mode);
...
static ssize_t event_path_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp6_core_data *cd = dev_get_drvdata(dev);
    unsigned long value;
...
static ssize_t event_path_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct cyttsp6_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
    ...
    if (cd->event_path)
        ret = snprintf(buf, CY_MAX_PRBUF_SIZE,
            "Touch event ENABLED for Native Application\n");
    else
        ret = snprintf(buf, CY_MAX_PRBUF_SIZE,
            "Touch event ENABLED for Android\n");
    mutex_unlock(&cd->system_lock);
...
static ssize_t touch_pos_z_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct cyttsp6_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
...
static struct device_attribute attributes[] = {
	...
	__ATTR(event_path, S_IRUSR | S_IWUSR, event_path_show,
			event_path_store),
	__ATTR(touch_pos_x, S_IRUGO, touch_pos_x_show, NULL),
	__ATTR(touch_pos_y, S_IRUGO, touch_pos_y_show, NULL),
	__ATTR(touch_pos_z, S_IRUGO, touch_pos_z_show, NULL),
};
...
// 	acpi_register_gsi(NULL, cd->irq, ACPI_LEVEL_SENSITIVE , ACPI_ACTIVE_LOW);
		...
		u16 irq, size_t xfer_buf_size)
{
...
#if 0
	rc = gpio_direction_output(453, 1);
    if(rc>=0){
        gpio_set_value(453,0);
        mdelay(100);
        gpio_set_value(453,1);
    }
	 ...
	 */
//SWPF test by commenting startup
