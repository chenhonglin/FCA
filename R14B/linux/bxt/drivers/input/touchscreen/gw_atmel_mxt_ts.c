...
#define CONFIG_R1_DISPLAY 1
#ifdef CONFIG_R1_DISPLAY
 /* Touch Interrupt */
 #define ATMEL_TOUCH_IRQ 0x71
#endif
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
	...
	if ((status & MXT_T6_STATUS_OFL) && !(status & MXT_T6_STATUS_CAL)) {
		dev_dbg(dev, "%s: OFL, recalibrate chip\n", __func__);
...
{
	if ((data->pdata->t19_num_keys) && (data->pdata->t19_keymap)) {
...
{
	if ((data->pdata->t15_num_keys) && (data->pdata->t15_keymap)) {
		struct input_dev *input_dev = data->input_dev;
		struct device *dev = &data->client->dev;
		int key;
...
static int mxt_initialize_t9_input_device(struct mxt_data *data)
{
	...
	if ((pdata->t19_num_keys) && (pdata->t19_keymap)) {
	...
	if ((pdata->t15_num_keys) && (pdata->t15_keymap)) {
		/* For T15 key array */
...
static void mxt_force_bootloader(struct mxt_data *data)
{
	...
	dev_dbg(&data->client->dev, "Force to enter bootloader\n");
	for (i = 0; i < 10; i++) {
		MSLEEP(1);
		gpio_set_value(data->pdata->gpio_reset, 1);
		MSLEEP(120);
				...
				const struct firmware *fw)
{
	...
	if (strncmp(fw->data + pos, MXT_FW_MAGIC, strlen(MXT_FW_MAGIC))) {
		dev_dbg(&data->client->dev, "Skip non supported firmware\n");
		return true;
	}
	...
	if (fw_info->family_id == 0xff && fw_info->variant_id == 0xff &&
		...
		dev_dbg(&data->client->dev, "Force update firmware\n");
		return false;
	}
	...
	if (fw_info->family_id == info->family_id &&
		...
		dev_dbg(&data->client->dev, "Update firmware\n");
		return false;
	} else
		return true;
}
...
retry_bootloader:
		...
		if (!error) {
			dev_dbg(&client->dev, "Firmware update succeeded\n");
...
static int mxt_initialize_with_fw_check(struct mxt_data *data)
{
	...
	dev_dbg(&data->client->dev, "call to request_firmware_nowait: *module:%p, fw_name:%s, *context:%p, *func_to_call_after_fw_request:%p\n", THIS_MODULE, data->fw_name, data, mxt_fw_cb);
		...
		const struct i2c_device_id *id)
{
	...
	dev_dbg(&client->dev, "data->pdata->gpio_int: %d\n", data->pdata->gpio_int);

	dev_dbg(&client->dev, "data->pdata1: %p\n", data->pdata);

#ifdef CONFIG_OF
	if (!data->pdata && client->dev.of_node)
		data->pdata = mxt_parse_dt(client);
#endif
	dev_dbg(&client->dev, "data->pdata2: %p\n", data->pdata);
...
#ifdef CONFIG_R1_DISPLAY
/* NOTE: this change was made in the wrong place on ww45+ gerrits.  This client-> IRQ doesn't get called in our environment. */
			client->irq = ATMEL_TOUCH_IRQ;
			acpi_register_gsi(NULL, client->irq, ACPI_LEVEL_SENSITIVE, ACPI_ACTIVE_LOW);
#else
			client->irq =
...
/*  NOTE: this acpi_register_gsi change should have been made here for the loss of touch IRQ issue.
*
*  However, if #define ATMEL_TOUCH_IRQ 0x71 is used as originally spelled out in the GM gerrit,
*  touch doesn't work as client->irq derived from pdata->gpio_int (434+14), results in a client->irq 0x86 address
*
*	So --  don't use this absolute IRQ address define
*
*	client->irq = ATMEL_TOUCH_IRQ;
*
*  do -- use the client->irq address, derived from pdata->gpio_int address.
*/
	client->irq = gpiod_to_irq(gpio_to_desc(data->pdata->gpio_int));
/*
*  do -- add the acpi_register_gsi() function call with derived client->irq address
*
*	acpi_register_gsi(NULL, client->irq, ACPI_LEVEL_SENSITIVE, ACPI_ACTIVE_LOW);
*/

	dev_dbg(&client->dev, "client->irq: %02x\n", client->irq);
#endif
	data->irq = client->irq;
	...
	dev_dbg(&client->dev, "call: mxt_probe regulators\n");
...
static int mxt_suspend(struct device *dev)
{
	...
	dev_dbg(&client->dev, "mxt_suspend complete\n");
...
static int mxt_resume(struct device *dev)
{
	...
	data->suspended = false;
	dev_dbg(&client->dev, "mxt_resume complete\n");
