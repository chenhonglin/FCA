		...
		struct device_node *core_node)
{
	...
	pdata->error_stat = cyttsp4_error_stat;
...
static struct cyttsp4_adapter_platform_data *create_and_get_adap_pdata(
		struct device_node *core_node)
{
	...
	adap_pdata = kzalloc(sizeof(*adap_pdata), GFP_KERNEL);
	if (!adap_pdata) {
		rc = -ENOMEM;
		return ERR_PTR(rc);
	}

	rc = of_property_read_u32(core_node, "cy,bl_i2c_client_addr", &value);
	if (rc)
		goto fail_free;
	adap_pdata->bl_i2c_client_addr = value;

	pr_debug("%s: bl_i2c_client_addr:0x%X\n", __func__,
		 adap_pdata->bl_i2c_client_addr);
...
static void free_adapter_pdata(void *pdata)
{
	struct cyttsp4_adapter_platform_data *adap_pdata = pdata;

	if (!IS_ERR_OR_NULL(adap_pdata))
		kfree(adap_pdata);
}
...
int cyttsp4_devtree_clean_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		if (!rc) {
			...
			} else if (!strcmp(name, "cy,adapter")) {
				free_adapter_pdata(pdata->adap_pdata);
				of_node_put(core_node);
			}
		}
	}
...
int cyttsp4_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		if (!strcmp(name, "cy,core")) {
			...
			pdata->adap_pdata =
					create_and_get_adap_pdata(core_node);
			if (IS_ERR(pdata->adap_pdata)) {
				rc = PTR_ERR(pdata->adap_pdata);
				break;
			}
			/* Increment reference count */
			of_node_get(core_node);
