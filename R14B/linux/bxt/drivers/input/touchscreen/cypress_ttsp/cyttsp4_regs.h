...
#define CY_CORE_WAKEUP_TIMEOUT			200
#else
...
#define CY_LDR_CMD_ERASE_TIMEOUT		500
#define CY_LDR_CMD_SEND_TIMEOUT			500
...
#define IS_BOOTLOADER(hst_mode, reset_detect)		((hst_mode) & 0x01)
#define IS_BOOTLOADER_IDLE(hst_mode, reset_detect) \
		((hst_mode) & 0x01 && (reset_detect) & 0x01)
#define IS_BOOTLOADER_ACTIVE(hst_mode)			((hst_mode) == 0x1)
/*FIXME*/
#ifdef GEN4
...
#define TTSP20_WAKEUP_IRQ_COUNT		3

/* DEVICE REGISTERS */
/* OP MODE REGISTERS */
#define CY_REG_BASE			0x00
#define CY_GPIO_BASE                    0x00
...
#define CY_CMD_COMPLETE_MASK		BIT(6)	/* (1 << 6) */
...
enum cyttsp4_ic_ebid {
	CYTTSP4_TCH_PARM_EBID,
	CYTTSP4_DDATA_EBID = 0x05,
	CYTTSP4_MDATA_EBID,
};

/* FLASH BLOCKS */
enum cyttsp5_ic_ebid {
	CYTTSP5_TCH_PARM_EBID,
	CYTTSP5_MDATA_EBID,
	CYTTSP5_DDATA_EBID,
};

/* ttconfig block */
#define CYTTSP4_TTCONFIG_VERSION_OFFSET	27
#define CYTTSP5_TTCONFIG_VERSION_OFFSET	8
#define CY_TTCONFIG_VERSION_SIZE	2
#define CY_TTCONFIG_VERSION_ROW		0

#define CY_CONFIG_LENGTH_INFO_OFFSET	0
/*
 * SOLO fw (tested with 2.0 and 2.1 versions) can only read/write 128 bytes
 * from/to a config block.
 * And also CRC calculation is performed for 126 bytes (max_size) not for
 * data len (size). CY_CONFIG_LENGTH_DATA_SIZE is used for this purpose.
 */
#define CY_CONFIG_LENGTH_DATA_SIZE	126
#define CY_CONFIG_LENGTH_ROW_SIZE	128
#define CYTTSP4_CONFIG_LENGTH_INFO_SIZE	128
#define CYTTSP5_CONFIG_LENGTH_INFO_SIZE	4
...
/* FW file name */
#define CYTTSP4_FW_FILE_NAME		"cyttsp4_fw.bin"
#define CYTTSP5_FW_FILE_NAME		"cyttsp5_fw.bin"
#define CYTTSP6_FW_FILE_NAME		"cyttsp6_fw.bin"
...
/* Core module */
#define CY_DEFAULT_CORE_ID0		"cyttsp4_core0"
#define CY_DEFAULT_CORE_ID1		"cyttsp4_core1"
...
struct cyttsp4_core_data {
	...
	bool bl_fast_exit;
	unsigned short wakeup_int_cnt;
	u8 bl_buf[CY_BL_MAX_STATUS_SIZE];
	int max_xfer;
	int apa_mc_en;
	int glove_en;
	int stylus_en;
...
struct cyttsp4_bus_ops {
	int (*write)(struct device *dev, u16 addr, u8 *wr_buf, const void *buf,
		     int size, int max_xfer, u16 bl_i2c_client_addr);
...
static inline int is_crc_stat_failed(struct cyttsp4_core_data *cd)
{
