...
static void cyttsp4_report_gesture(struct cyttsp4_mt_data *md, int gest_id)
{
	input_event(md->input, EV_MSC, MSC_GESTURE, gest_id);
}

static void cyttsp4_report_noise(struct cyttsp4_mt_data *md, int noise)
{
	...
	for (i = 0; i < num_cur_rec; i++) {
		...
		if (IS_TTSP_VER_GE(si, 2, 3)) {
			...
			if (tch.abs[CY_TCH_P] > 0 && tch.abs[CY_TCH_MAJ] == 0) {
				tch.abs[CY_TCH_MAJ] = 1;
				tch.abs[CY_TCH_MIN] = 1;
			}
...
static int cyttsp4_xy_worker(struct cyttsp4_mt_data *md)
{
	...
	if (gest_id)
		cyttsp4_report_gesture(md, gest_id);
	if (gest_pos_x || gest_pos_y)
		cyttsp4_report_gesture(md, (gest_pos_x << 16) | gest_pos_y);
	if (noise_wideband || noise_level)
		cyttsp4_report_noise(md, (noise_wideband << 8) | (noise_level));
...
int cyttsp4_mt_probe(struct device *dev)
{
	...
	if (md->pdata->inp_dev_name == CYTTSP4_MT_NAME_VTP)
		md->input->name = CYTTSP4_MT_NAME_VTP;
	else if (md->pdata->inp_dev_name == CYTTSP4_MT_NAME_IVI)
		md->input->name = CYTTSP4_MT_NAME_IVI;
	else
		md->input->name = CYTTSP4_MT_NAME;
