...
#define VIRTUAL_KEY_MAP_FILE_NAME "virtualkeys." PLATFORM_DRIVER_NAME_IVI
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	rmi4_data->hw_if->bus_access->read_isr(rmi4_data);
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		...
		printk("%s:rmi4_data->irq=%d\n",__func__,rmi4_data->irq);
		retval = request_threaded_irq(rmi4_data->irq, NULL,
				synaptics_rmi4_irq, bdata->irq_flags,
				PLATFORM_DRIVER_NAME_IVI, rmi4_data);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->input_dev->name = PLATFORM_DRIVER_NAME_IVI;
...
void synaptics_rmi4_new_function_ivi(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
EXPORT_SYMBOL(synaptics_rmi4_new_function_ivi);
...
static int __init synaptics_rmi4_init(void)
{
	...
	retval = synaptics_rmi4_bus_ivi_init();
	if (retval)
		return retval;
