...
#define CRL_REG_READ_AND_UPDATE_MOPAR           (1 << 5)
#define CRL_REG_LEN_READ_MASK_MOPAR             0x0f
#endif
...
struct crl_sensor_configuration {
	...
	const unsigned int status_regs_items;
	const struct crl_register_read_rep *status_regs;
	...
	struct crl_sensor_limits *sensor_limits;
...
#ifdef CONFIG_INTEL_IPU4_DS90UB954_MOPAR
        // To allow dynamically switching 'modes' based on camera type until we
        // can resolve why multiple modes in the same struct does not work as
        // expected
        void (*get_modes)(struct i2c_client*, struct crl_mode_rep **modes, unsigned int *modes_items);
        struct crl_sensor_limits * (*get_limits)(struct i2c_client*);
#endif
	/* Harman added callbacks for custom 'streamon/streamoff' behavior that
	 * cannot be handled by simple streamoff_regs, streamon_regs lists. 
	 * sensor->mutex must be locked prior to entry. return 0 on success. */
	int (*streamon_callback)(struct i2c_client* client);
	int (*streamoff_callback)(struct i2c_client* client);

	/* Harman extension for module specific suspend & resume sequences.
	 * suspend callback to be invoked BEFORE unwinding the poweron
	 * items (if registered) */
	int (*suspend_callback)(struct i2c_client *client);
	int (*resume_callback)(struct i2c_client *client);

	/* callback to get sensor-specific status */
	int (*sensor_status)(struct i2c_client* client, int *status);
};
...
/* Function to lookup crlmodule_sensors[N].name from crl_sensor.sensor_ds */
