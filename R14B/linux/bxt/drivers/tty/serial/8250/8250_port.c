...
#ifdef CONFIG_BJEVN60_ED2_BOARD
			/* UART port 3 is used for IPC in BJEV */
			if (serial_index(port) != 3)
				serial_port_out(port, UART_FCR, up->fcr);	/* set fcr */
			else
				serial_port_out(port, UART_FCR, UART_FCR_ENABLE_FIFO | UART_FCR_R_TRIG_01);
#else
			serial_port_out(port, UART_FCR, up->fcr);	/* set fcr */
#endif
