...
static struct cyttsp4_mt_platform_data _cyttsp4_mt_vtp_platform_data = {
...
static struct bjev_vtp_display_platform_data bjev_vtp_pdata = {
	...
	.cypress_platform_data  = {
		...
		.mt_pdata = &_cyttsp4_mt_vtp_platform_data,
...
//	{ I2C_BOARD_INFO("i2c_ipc_vtp", BJEV_VTP_DU_ADDR),
//	},
};
...
//	{ I2C_BOARD_INFO("i2c_ipc", BJEV_IVI_DU_ADDR),
//	},
};
...
static int apl_i2c_board_setup(void)
{
    ...
    if (ret)
        pr_err(DRVNAME "failed to register IVI serializer (ret: %d)\n",ret);
    else
        pr_err(DRVNAME "successfully registered IVI serializer..\n");
