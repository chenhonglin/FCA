...
woal_drv_dump_read(struct seq_file *sfp, void *data)
{
	...
	memcpy(sfp->buf, handle->drv_dump_buf, handle->drv_dump_len);
done:
...
woal_fw_dump_read(struct seq_file *sfp, void *data)
{
	...
	memcpy(sfp->buf, handle->fw_dump_buf, handle->fw_dump_len);
...
woal_proc_init(moal_handle *handle)
{
	...
	if (!handle->proc_mwlan) {
		...
		if (handle->proc_mwlan) {
			...
			if (handle->handle_idx)
				sprintf(drv_dump_dir, "drv_dump%d",
					handle->handle_idx);
			else
				strcpy(drv_dump_dir, "drv_dump");
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 26)
			r = proc_create_data(drv_dump_dir, 0644,
					     handle->proc_mwlan, &drv_dump_fops,
					     handle);
			if (r == NULL)
#else
			r = create_proc_entry(drv_dump_dir, 0644,
					      handle->proc_mwlan);
			if (r) {
				r->data = handle;
...
#endif
				PRINTM(MERROR,
				       "Failed to create proc drv dump\n");

			if (handle->handle_idx)
				sprintf(fw_dump_dir, "fw_dump%d",
					handle->handle_idx);
			else
				strcpy(fw_dump_dir, "fw_dump");
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 26)
			r = proc_create_data(fw_dump_dir, 0644,
					     handle->proc_mwlan, &fw_dump_fops,
					     handle);
			if (r == NULL)
#else
			r = create_proc_entry(fw_dump_dir, 0644,
					      handle->proc_mwlan);
			if (r) {
				r->data = handle;
...
woal_proc_exit(moal_handle *handle)
{
	...
	if (handle->proc_mwlan) {
		...
		if (handle->handle_idx)
			sprintf(drv_dump_dir, "drv_dump%d", handle->handle_idx);
		else
			strcpy(drv_dump_dir, "drv_dump");
		remove_proc_entry(drv_dump_dir, handle->proc_mwlan);
		if (handle->handle_idx)
			sprintf(fw_dump_dir, "fw_dump%d", handle->handle_idx);
		else
			strcpy(fw_dump_dir, "fw_dump");
		remove_proc_entry(fw_dump_dir, handle->proc_mwlan);
		if (handle->handle_idx)
