...
int m88q_phy = 0;
int m88q_ge_flag = 0;
module_param(m88q_phy, int, 0);
module_param(m88q_ge_flag, int, 0);
MODULE_PARM_DESC(m88q_phy, "switch to enable marvell88q phy support (0=all except M88Q_PHY, 1=M88Q_PHY)");
MODULE_PARM_DESC(m88q_ge_flag, "switch to marvell88q phy ge or fe (0=all except M88Q_PHY_GE, 1=M88Q_PHY_GE)");
...
static int igb_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (pdev->is_virtfn) {
		...
		printk(KERN_INFO "%s (%hx:%hx) should not be a VF!\n",
			pci_name(pdev), pdev->vendor, pdev->device);
