 ...
 * Supported parts families include:
 * GEN6 XL
 * GEN L
 *
...
static const u8 gen3_security_key[] = {
	0x45, 0x63, 0x36, 0x6F, 0x34, 0x38, 0x73, 0x77
};

static const u8 gen3_ldr_exit[] = {
	CY_START_OF_PACKET, CY_CMD_LDR_EXIT, 0x00, 0x00, 0x4F, 0x6D,
	CY_END_OF_PACKET
};

static const u8 gen3_ldr_err_app[] = {
	CY_START_OF_PACKET, 0x0F, 0x00, 0x00, 0xAA, 0xA2, CY_END_OF_PACKET
};
...
MODULE_FIRMWARE(CYTTSP4_FW_FILE_NAME);
MODULE_FIRMWARE(CYTTSP5_FW_FILE_NAME);
MODULE_FIRMWARE(CYTTSP6_FW_FILE_NAME);
...
static inline int cyttsp4_adap_read(struct cyttsp4_core_data *cd, u16 addr,
				    void *buf, int size)
{
	return cd->bus_ops->read(cd->dev, addr, buf, size, cd->max_xfer, 0);
}
...
static inline int cyttsp4_adap_bl_read(struct cyttsp4_core_data *cd, u16 addr,
				       void *buf, int size)
{
	return cd->bus_ops->read(cd->dev, addr, buf, size, cd->max_xfer,
			cd->pdata->adap_pdata->bl_i2c_client_addr);
}

static inline int cyttsp4_adap_bl_write(struct cyttsp4_core_data *cd, u16 addr,
					const void *buf, int size)
{
	return cd->bus_ops->write(cd->dev, addr, cd->wr_buf, buf, size,
				  cd->max_xfer,
...
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	u8 const *key = security_key;

	if (cd->dev_id == CY_DEVID_GEN3)
		key = gen3_security_key;
					       ...
					       bool optimize)
{
	...
	cd->gpio_data = si->xy_mode[si->si_ofs.tt_stat_ofs-1];
	dev_vdbg(dev, "%s: %s%02X %s%d %s%02X %s%02X\n", __func__,
		 "hst_mode=", hst_mode, "rep_len=", rep_len,
		 "rep_stat=", rep_stat, "tt_stat=", tt_stat);
	if ((cd->dev_id & GEN6_DEVICE) &&
			IS_TTSP_VER_GE(&cd->sysinfo, 2, 5)) {
		dev_dbg(dev, "%s: GPIO READ SUCCESS:GPIO_1_2:%01X GPIO_1_5:%01X\n",
		__func__, (cd->gpio_data>>2)&1, (cd->gpio_data>>5)&1);
	}
...
static int cyttsp4_hw_soft_reset_(struct cyttsp4_core_data *cd)
{
	...
	rc = cyttsp4_adap_write(cd, CY_REG_BASE, &cmd, sizeof(cmd));
	if (rc < 0) {
		rc = cyttsp4_adap_bl_write(cd, CY_REG_BASE, &cmd, sizeof(cmd));
		if (rc < 0) {
...
static int cyttsp4_si_get_test_data_(struct cyttsp4_core_data *cd)
{
	...
	if (cd->dev_id != CY_DEVID_GEN3) {
		...
		dev_info(cd->dev, "%s: SCANNING is %s codel=%02X\n", __func__,
			 ...
			 CY_POST_CODEL_SCAN_STATUS ? "ENABLED" : "DISABLED",
			 si->si_ptrs.test->post_codel);
	}
...
static int cyttsp4_si_get_opcfg_data_(struct cyttsp4_core_data *cd)
{
	...
	memset(si->si_ptrs.opcfg, 0x0, si->si_ofs.opcfg_size);
	...
	/*
	 * There is no TOUCH_MAJOR, TOUCH_MINOR and ORIENTATION fields for
	 * TTSP v2.0. Thus, it is required to clear these fields.
	 */
	memset(&si->si_ofs.tch_abs, 0x0, sizeof(si->si_ofs.tch_abs));
...
static inline void cyttsp4_read_active_bl_resp(struct cyttsp4_core_data *cd,
					       u8 *mode, int mode_size)
{
	int rc;
	int len;

	memcpy(cd->bl_buf, mode, mode_size);

	/*
	 * mode[2] is data_len(lsb).
	 * data_len(msb) is always 0 in bl command replies.
	 * need to read mode[2] + 4 bytes more.
	 * data_len(msb) + crc(msb) + crc(lsb) + packet_end = 4.
	 */
	len = mode[2] + 4;
	rc = cyttsp4_adap_bl_read(cd, CY_REG_BASE, &cd->bl_buf[mode_size], len);
	if (rc < 0)
		dev_err(cd->dev, "%s: Fail read adapter rc=%d\n", __func__, rc);
}
...
//	bjev_vtp_ser_read_isr();
	...
	mutex_lock(&cd->system_lock);
        ...
        if(cd->dev_id == CY_DEVID_GEN6) {
                //dev_err(dev, "%s int: GEN6\n", __func__);
                bjev_ivi_ser_read_isr();

        } else if (cd->dev_id == CY_DEVID_GEN6_2 ){
                //dev_err(dev, "%s int: GEN6_2\n", __func__);
                bjev_vtp_ser_read_isr();
        }
	...
	if (cd->dev_id == CY_DEVID_GEN3) {
		/* fw sends 3 irqs in waking up. */
		if ((cd->int_status & CY_INT_AWAKE) ||
		    (cd->int_status & CY_INT_IGNORE)) {
			if (++cd->wakeup_int_cnt == TTSP20_WAKEUP_IRQ_COUNT)
				cd->wakeup_int_cnt = 0;
			else
		...
		/*
		 * do NOT remove this case, otherwise fw upgrade will be broken.
		 * there is no address info in i2c read operation of bl mode.
		 * writing i2c bl addr in reading causes bl to exit from active
		 * mode.
		 */
		if (cd->mode == CY_MODE_BOOTLOADER ||
		    cd->mode == CY_MODE_UNKNOWN) {
			rc = cyttsp4_adap_bl_read(cd, CY_REG_BASE, mode,
						  sizeof(mode));
			if (!rc) {
				if (IS_BOOTLOADER_ACTIVE(mode[0]))
					cyttsp4_read_active_bl_resp(cd, mode,
								    sizeof(mode)
								    );
			} else {
				/* Required for BL to APP switch */
				rc = cyttsp4_adap_read(cd, CY_REG_BASE, mode,
						       sizeof(mode));
			}
		} else {
			rc = cyttsp4_adap_read(cd, CY_REG_BASE, mode,
					       sizeof(mode));
			if (rc < 0) /* Required to detect BL glitch */
				rc = cyttsp4_adap_bl_read(cd, CY_REG_BASE, mode,
							  sizeof(mode));
		}
	} else {
		   ...
		   int size)
{
	...
	if (mode == CY_MODE_BOOTLOADER)
		return cyttsp4_adap_bl_write(cd, addr, buf, size);
			    ...
			    size_t param_ret_size, int timeout_ms)
{
	...
	if (param_ret_size == 0 || !return_buf)
		return 0;
...
static int cyttsp4_request_exec_cmd_(struct device *dev, u8 mode,
				     u8 *cmd_buf, size_t cmd_size,
				     u8 *return_buf, size_t param_ret_size,
				     int timeout_ms)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
				 ...
				 u8 param_size, u32 param_value)
{
	...
	if (cd->dev_id == CY_DEVID_GEN3) {
		dev_err(cd->dev, "%s: set parameter not supported\n",
			__func__);
		return -EINVAL;
	}
				      ...
				      u16 row, const u8 *data, u16 length)
{
	...
	if (cd->dev_id == CY_DEVID_GEN3) {
		command_buf[CY_CMD_CAT_WRITE_CFG_BLK_CMD_HDR_SZ
				+ length] = HI_BYTE(crc);
		command_buf[CY_CMD_CAT_WRITE_CFG_BLK_CMD_HDR_SZ
				+ length + 1] = LO_BYTE(crc);
		length += 2;
		memcpy(&command_buf[
			CY_CMD_CAT_WRITE_CFG_BLK_CMD_HDR_SZ + length],
			gen3_security_key, sizeof(gen3_security_key));
	} else {
...
static int cyttsp4_get_ttconfig_props(struct cyttsp4_core_data *cd,
				      u16 *version, u16 *length,
				      u16 *max_length)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	u8 data[CYTTSP4_CONFIG_LENGTH_INFO_SIZE];
	int rc;

	rc = cyttsp4_read_config_block(cd, CYTTSP4_TCH_PARM_EBID,
	...
	*version = GET_FIELD16(si, &data[CYTTSP4_TTCONFIG_VERSION_OFFSET]);
...
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	u8 data[CYTTSP5_CONFIG_LENGTH_INFO_SIZE];
	int rc;
...
{
	u8 row_data[CY_CONFIG_LENGTH_ROW_SIZE] = {0U};
	u8 data_size, max_size;
	int rc;

	/* 4 bytes for size and max_size info. */
	if (length > (CY_CONFIG_LENGTH_DATA_SIZE - 4)) {
		dev_err(cd->dev, "%s: length cannot be greater than %d\n",
			__func__, CY_CONFIG_LENGTH_DATA_SIZE - 4);
		return -EINVAL;
	}

	if (offset) {
		rc = cyttsp4_read_config_block(cd, ebid, 0, row_data,
					       sizeof(row_data));
		if (rc) {
			dev_err(cd->dev, "%s: Error on read config block\n",
				__func__);
		...
		/*
		 * get actual data size stored in the row.
		 * row_data = { size(msb), size(lsb),
		 *              max_size(msb), max_size(lsb), ... }
		 */
		data_size = get_unaligned_be16(row_data);
		max_size = get_unaligned_be16(&row_data[2]);

		/*
		 * 1st 4 bytes are size and max_size info. they should not be
		 * changed. and offset should be lower than data size also.
		 */
		if ((offset < 4) || (offset > (data_size - 1))) {
			dev_err(cd->dev, "%s: invalid offset value\n",
				__func__);
			return -EINVAL;
		}

		memcpy(&row_data[offset], data, length);
		rc = cyttsp4_write_config_block(
			cd, ebid, 0, row_data, max_size);
	} else {
		/*
		 * SOLO fw (tested with 2.0 and 2.1 versions)
		 * can only write 128 bytes to a config block.
		 * And also CRC calculation is performed for 126
		 * bytes (max_size) not for data len (size).
		 * CY_CONFIG_LENGTH_DATA_SIZE is used
		 * for this purpose.
		 */

		/* Driver.h does not include size and max_size info. */
		row_data[0] = HI_BYTE(length + 4);
		row_data[1] = LO_BYTE(length + 4);
		row_data[2] = HI_BYTE(CY_CONFIG_LENGTH_DATA_SIZE);
		row_data[3] = LO_BYTE(CY_CONFIG_LENGTH_DATA_SIZE);
		memcpy(&row_data[4], data, length);
		rc = cyttsp4_write_config_block(cd, ebid, 0, row_data,
						CY_CONFIG_LENGTH_DATA_SIZE);
	}
...
static int cyttsp5_write_config_common(struct cyttsp4_core_data *cd, u8 ebid,
				       u16 offset, u8 *data, u16 length)
{
...
static int cyttsp5_write_config(struct cyttsp4_core_data *cd, u8 ebid,
				u16 offset, u8 *data, u16 length)
{
	int rc;
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	...
	rc = cyttsp5_write_config_common(cd, ebid, offset, data, length);
	if (rc) {
	...
	rc = cyttsp5_write_config_common(cd, ebid, crc_offset, crc_data, 2);
	if (rc)
					 ...
					 u16 offset, u8 *data, u16 length)
{
	...
	if (cd->dev_id == CY_DEVID_GEN3)
		...
		return cyttsp5_write_config(cd, ebid, offset, data, length);
}
...
static int _cyttsp4_put_device_into_easy_wakeup(struct cyttsp4_core_data *cd)
{
	...
	rc = wait_event_timeout(cd->wait_q,
				(cd->int_status & CY_INT_EXEC_CMD) == 0,
				 msecs_to_jiffies(
				 CY_CORE_CMD_WAIT_FOR_EVENT_TIMEOUT));
	mutex_lock(&cd->system_lock);
	if (IS_TMO(rc)) {
		cd->int_status &= ~CY_INT_EXEC_CMD;
		return rc;
...
static int _exit_bootloader(struct cyttsp4_core_data *cd)
{
	...
	if ((cd->cpdata->flags & CY_CORE_FLAG_BOOTLOADER_FAST_EXIT) &&
	    cd->bl_fast_exit && (cd->dev_id != CY_DEVID_GEN3)) {
		...
		dev_vdbg(cd->dev, "%s: Device ID:%d\n", __func__, cd->dev_id);
		if (cd->dev_id == CY_DEVID_GEN3) {
			exit_cmd = gen3_ldr_exit;
			exit_cmd_size = sizeof(gen3_ldr_exit);
		} else {
			exit_cmd = ldr_exit;
	...
	rc = cyttsp4_adap_bl_write(cd, CY_REG_BASE, exit_cmd, exit_cmd_size);
	if (rc < 0)
...
static int cyttsp4_read_gesture_enable(struct cyttsp4_core_data *cd,
				       u8 *gesture_enable){
	int rc, rowid, offset;
	u8 *row_data;
	u16 config_row_size;
	...
	rc = cyttsp4_read_config_block(cd, CYTTSP5_TCH_PARM_EBID, rowid,
				       row_data, offset);
	if (rc < 0) {
...
static int cyttsp4_get_ttconfig_info(struct cyttsp4_core_data *cd)
{
	...
	if (cd->dev_id == CY_DEVID_GEN3) {
		rc = cyttsp4_get_ttconfig_props(cd,
		...
		rc = cyttsp4_get_config_length(cd, CYTTSP5_TCH_PARM_EBID,
					       &length, &max_length);
	}
	...
	if (cd->dev_id & GEN6_DEVICE) {
		rc = cyttsp4_read_gesture_enable(cd, &gesture_enable);
		if (rc < 0) {
	...
	rc = cyttsp4_get_config_block_crc(cd, CYTTSP4_TCH_PARM_EBID, &crc);
	if (rc < 0) {
		dev_err(cd->dev, "%s: failed to get ttconfig crc rc=%d\n",
			__func__, rc);
		return rc;
...
reset:
	...
	if (rc < 0) {
		...
		if (cd->dev_id != CY_DEVID_GEN3) {
			...
			ldr_err = ldr_err_app;
			rc1 = cyttsp4_adap_read(cd, CY_REG_BASE, cd->bl_buf,
						sizeof(ldr_err_app));
			if (rc1) {
			...
			ldr_err = gen3_ldr_err_app;
		}
		if (!memcmp(cd->bl_buf, ldr_err, sizeof(ldr_err_app))) {
			cyttsp4_pr_buf(cd->dev, cd->pr_buf, cd->bl_buf,
				       sizeof(ldr_err_app), "bl_buf2");
	...
	if (cd->dev_id != CY_DEVID_GEN3) {
		if (cd->cpdata->flags &
			CY_CORE_FLAG_SCAN_MODE_USES_RAM_ID_SCAN_TYPE)
...
/*
 *Show GPIO Byte Status
 *
*/
static ssize_t cyttsp4_gpio_byte_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	return snprintf(buf, CY_MAX_PRBUF_SIZE, "%02X\n", cd->gpio_data);
}

/*
 *Show GPIO P[1].2 Bit Status
 *
*/
static ssize_t cyttsp4_gpio_1_2_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
...
/*
 *Show GPIO P[1].5 Bit Status
 *
*/
static ssize_t cyttsp4_gpio_1_5_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
 ...
 */
static ssize_t cyttsp4_hw_error_stat_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	...
	if (cd->cpdata->error_stat) {
		...
		case 0:
			return snprintf(buf, CY_MAX_PRBUF_SIZE,
						"Error line is LOW.\n");
		case 1:
			rc = cyttsp4_adap_read(cd, CY_ERR_OFFSET, error, 2);
			if (rc < 0) {
...
static struct device_attribute attributes[] = {
	...
	__ATTR(GPIO_Status, S_IRUSR, cyttsp4_gpio_byte_show, NULL),
	__ATTR(Pin_1_2_Status, S_IRUSR, cyttsp4_gpio_1_2_show, NULL),
	__ATTR(Pin_1_5_Status, S_IRUSR, cyttsp4_gpio_1_5_show, NULL),
	__ATTR(hw_error_stat, S_IRUSR, cyttsp4_hw_error_stat_show, NULL),
		  ...
		  u16 irq, enum cy_dev_id dev_id)
{
	...
	if ((cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) &&
	    !cd->cpdata->power) {
		dev_err(dev,
			"%s: No power function with POWEROFF_ON_SLEEP flag\n",
			__func__);
		rc = -EINVAL;
		goto error_power;
	}
...
#endif
	/*probe success; core_ids is available for other modules*/
