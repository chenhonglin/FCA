...
#define CYTTSP4_DATA_ROW_SIZE			288
#define CYTTSP5_DATA_ROW_SIZE			128
#define CYTTSP4_IMAGE_REC_SIZE			(CYTTSP4_DATA_ROW_SIZE + 5)
#define CYTTSP5_IMAGE_REC_SIZE			(CYTTSP5_DATA_ROW_SIZE + 5)
...
#define CY_COMPUTE_CRC(dev, buf, len) \
	_cy_compute_crc(dev, buf, len)
...
struct cyttsp4_loader_data {
	...
	/* lock to load and update config */
...
static inline char *cyttsp4_get_fw_bin_name(
		enum cy_dev_id dev_id)
{
	if (dev_id == CY_DEVID_GEN3)
		return CYTTSP4_FW_FILE_NAME;
	else if (dev_id == CY_DEVID_GEN4)
		return CYTTSP5_FW_FILE_NAME;
	else if (dev_id & GEN6_DEVICE)
		return CYTTSP6_FW_FILE_NAME;
	else
		return NULL;
}
...
static u16 _cy_compute_crc(struct device *dev, const u8 *buf, int size)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);

	if (cd->dev_id == CY_DEVID_GEN3)
		return _cyttsp4_compute_crc(buf, size);
	else
		return _cyttsp4_compute_crc(&buf[1], size - 1);
}
			       ...
			       unsigned long timeout_ms)
{
	...
	if (timeout_ms != 0) {
		...
		if (cd->dev_id == CY_DEVID_GEN3) {
			/*
			 * There is no address parameter in bootloader mode
			 * of SOLO. Thus, loader needs irq handler of core
			 * module to read whole result of the executed
			 * bootloader command.
			 * bl_buf = { packet start, status, data len (lsb),
			 *            data len (msb), data, crc (lsb),
			 *            crc (msb), packet end }
			 */
			memcpy(buf, cd->bl_buf, size);
		} else {
			/* TODO: Reconsider purpose of having retries here */
			for (tries = 0; tries < 2; tries++) {
				retval = cmd->read(dev, CY_MODE_BOOTLOADER,
						CY_REG_BASE, buf, size);
				/*
				 * retry if bus read error or
				 * status byte shows not ready
				 */
				if (buf[1] == CY_COMM_BUSY ||
				    buf[1] == CY_CMD_BUSY)
					msleep(20);
					/* TODO: Constant if code kept */
				else
					break;
			}
...
static int cyttsp4_add_host_sync(struct device *dev, u8 *buf, int i)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
...
static int _cyttsp4_ldr_enter(struct device *dev, struct cyttsp4_dev_id *dev_id)
{
	...
	i = cyttsp4_add_host_sync(dev, ldr_enter_cmd, i);
	...
	crc = CY_COMPUTE_CRC(dev, ldr_enter_cmd, i);
...
static int _cyttsp4_ldr_init(struct device *dev)
{
	...
	i = cyttsp4_add_host_sync(dev, ldr_init_cmd, i);
...
struct cyttsp4_hex_image {
	...
	u16 row_num;
	u16 row_size;
	u8 row_data[CYTTSP4_DATA_ROW_SIZE];
} __packed;
				 ...
				 struct cyttsp4_hex_image *row_image)
{
	...
	if (cd->dev_id == CY_DEVID_GEN3)
		/* Not sending full row, rest sent by ldr_send_data */
		data_len = CY_PACKET_DATA_LEN;
	else
		data_len = CYTTSP5_DATA_ROW_SIZE;

	next = cyttsp4_add_host_sync(dev, cmd, next);
	...
	crc = CY_COMPUTE_CRC(dev, cmd, next);
	cmd[next++] = (u8)crc;
	cmd[next++] = (u8)(crc >> 8);
				   ...
				   struct cyttsp4_hex_image *row_image)
{
	...
	i = cyttsp4_add_host_sync(dev, ldr_verify_row_cmd, i);
	...
	crc = CY_COMPUTE_CRC(dev, ldr_verify_row_cmd, i);
				      ...
				      u8 *app_chksum)
{
	...
	i = cyttsp4_add_host_sync(dev, ldr_verify_chksum_cmd, i);
	...
	crc = CY_COMPUTE_CRC(dev, ldr_verify_chksum_cmd, i);
...
static int _cyttsp4_ldr_exit(struct device *dev)
{
	...
	i = cyttsp4_add_host_sync(dev, ldr_exit_cmd, i);
	...
	crc = CY_COMPUTE_CRC(dev, ldr_exit_cmd, i);
...
static int _cyttsp4_ldr_erase_row(struct device *dev,
				  struct cyttsp4_hex_image *row_image)
{
	u16 crc;
	int i = 0;
	int retval = 0;
	int tries;
	u8 ldr_erase_row_cmd[CY_CMD_LDR_ERASE_ROW_CMD_SIZE];
	...
	crc = _cyttsp4_compute_crc(ldr_erase_row_cmd, i);
	...
	tries = 0;
	do {
		retval = _cyttsp4_send_cmd(dev, ldr_erase_row_cmd, i, NULL, 0,
					   CY_CMD_LDR_ERASE_ROW_STAT_SIZE,
					   CY_LDR_CMD_ERASE_TIMEOUT);
		if (retval < 0) {
			dev_err(dev, "%s: Fail erase row=%d r=%d\n",
				__func__, row_image->row_num, retval);
		}
...
static int _cyttsp4_ldr_send_data(struct device *dev, int *row_data_idx,
				  struct cyttsp4_hex_image *row_image)
{
	int retval = 0;
	int index = 0;
	...
	for (segment = 0; segment < (CYTTSP4_DATA_ROW_SIZE /
					CY_PACKET_DATA_LEN) - 1; segment++) {
		next = 0;
		cmd[next++] = CY_START_OF_PACKET;
		...
		for (data = 0; data < CY_PACKET_DATA_LEN; data++) {
			cmd[next] = row_image->row_data[index++];
			row_sum += cmd[next];
			next++;
		}

		crc = _cyttsp4_compute_crc(cmd, next);
		cmd[next++] = (u8)crc;
		cmd[next++] = (u8)(crc >> 8);
		cmd[next++] = CY_END_OF_PACKET;
		...
		do {
			retval =
			_cyttsp4_send_cmd(dev, cmd, next, NULL, 0,
					  CY_CMD_LDR_SEND_DATA_STAT_SIZE,
					  CY_LDR_CMD_SEND_TIMEOUT);
			if (retval < 0) {
				dev_err(dev, "%s: send row=%d segment=%d fail r=%d\n",
...
static int _cyttsp4_load_app(struct device *dev, const u8 *fw, int fw_size)
{
	...
	size_t row_buf_size = CY_MAX_PRBUF_SIZE < 1024 ?
		1024 : CY_MAX_PRBUF_SIZE;
	int row_count = 0;

	if (cd->dev_id == CY_DEVID_GEN3)
		image_rec_size = CYTTSP4_IMAGE_REC_SIZE;
	else
		image_rec_size = CYTTSP5_IMAGE_REC_SIZE;
	...
	row_image = kzalloc(sizeof(*row_image), GFP_KERNEL);
	file_id = kzalloc(sizeof(*file_id), GFP_KERNEL);
	dev_id = kzalloc(sizeof(*dev_id), GFP_KERNEL);
	...
	dev_info(dev, "%s: Wait for controller boot to BL\n", __func__);
	msleep(60);
	...
	while (p < (fw + fw_size)) {
		...
		if (cd->dev_id == CY_DEVID_GEN3) {
			/* erase row */
			retval = _cyttsp4_ldr_erase_row(dev, row_image);
			if (retval < 0) {
				dev_err(dev, "%s: Erase Row Error (array=%d row=%d ret=%d)\n",
					__func__, row_image->array_id,
					row_image->row_num, retval);
				goto _cyttsp4_load_app_exit;
			}
			...
			/* send data */
			retval = _cyttsp4_ldr_send_data(dev, &row_data_idx,
							row_image);
			if (retval < 0) {
				dev_err(dev, "%s: Send Data Error (array=%d row=%d ret=%d)\n",
					__func__, row_image->array_id,
					row_image->row_num, retval);
				goto _cyttsp4_load_app_exit;
			}
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
	...
	name = cyttsp4_get_fw_bin_name(cd->dev_id);
	if (!name) {
		dev_err(dev, "%s: No firmware name specified\n",
			__func__);
		return -EINVAL;
	}

	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
					 name, dev, GFP_KERNEL, dev,
				    ...
				    const u8 *ttconfig_data, int ttconfig_size)
{
	...
	rc = cmd->request_write_config(dev, CYTTSP4_TCH_PARM_EBID,
...
static int cyttsp4_get_ttconfig_crc(struct device *dev,
				    ...
				    u16 *crc)
{
	u8 data[CY_CONFIG_LENGTH_DATA_SIZE] = {0};

	/* 4 bytes for size and max_size info. */
	if (ttconfig_size > (CY_CONFIG_LENGTH_DATA_SIZE - 4))
		return -EOVERFLOW;

	/* Driver.h does not include size and max_size info. */
	data[0] = HI_BYTE(ttconfig_size + 4);
	data[1] = LO_BYTE(ttconfig_size + 4);
	data[2] = HI_BYTE(CY_CONFIG_LENGTH_DATA_SIZE);
	data[3] = LO_BYTE(CY_CONFIG_LENGTH_DATA_SIZE);
	memcpy(&data[4], ttconfig_data, ttconfig_size);

	*crc = cmd->calc_app_crc(data, CY_CONFIG_LENGTH_DATA_SIZE);
...
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(ld->dev);
	u16 ttconfig_offset;

	if (cd->dev_id == CY_DEVID_GEN3) {
		/* Driver.h file does not include size and max_size. */
		ttconfig_offset = CYTTSP4_TTCONFIG_VERSION_OFFSET - 4;
	} else {
		ttconfig_offset = CYTTSP5_TTCONFIG_VERSION_OFFSET;
	}
					  ...
					  u8 **start, int *len)
{
	...
	/*
	 * We need 11 bytes for FW version control info and at least 6 bytes
	 * in config (Length + Max Length + CRC). Note that SOLO does not
	 * include Length, Max Length and CRC in its TTConfig file.
	 */
	header_size = bin_config_data[0] + 1;

	if (cd->dev_id != CY_DEVID_GEN3 &&
	    (header_size < 11 ||
	    header_size >= bin_config_size - 6)) {
	...
	if (cd->dev_id == CY_DEVID_GEN3) {
		/*
		 * 4 bytes are added since Driver.bin does not include size
		 * and max_size bytes.
		 */
		config_size = bin_config_size - 11 + 4;
	} else {
