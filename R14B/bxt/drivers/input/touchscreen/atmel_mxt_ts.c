...
static irqreturn_t mxt_process_messages_t44(struct mxt_data *data)
{
	...
	if (ret) {
		...
		return IRQ_HANDLED; //temporary workaround
#else
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pdata = kzalloc(sizeof(struct mxt_platform_data),
					GFP_KERNEL);
	if (!pdata) {
...
//	data->irq = client->irq;
	...
	{
		client->irq = gpiod_to_irq(gpio_to_desc(441));
		data->irq = client->irq;
		printk("I2C Bus number = %x\n", client->adapter->nr);
	}
	else if (client->adapter->nr == 6) //for GWMV3
	{
		client->irq = gpiod_to_irq(gpio_to_desc(397));
		data->irq = client->irq;
		printk("I2C Bus number = %x\n", client->adapter->nr);
	}
	else
	{
		client->irq = gpiod_to_irq(gpio_to_desc(448));
		data->irq = client->irq;
		printk("I2C Bus number = %x\n", client->adapter->nr);
	}
