			       ...
			       u16 reg, u8 len, u32 val)
{
	...
	dev_dbg(&client->dev, "%s len reg, val: [%d, dev=%02x reg=0x%04x, addr=0x%04x]",
			       __func__, len, msg.addr, reg, val);
			...
			u8 len, u32 mask, u32 val)
{
	...
	if (len & CRL_REG_READ_AND_UPDATE_MOPAR) {
#else
...
#ifdef CONFIG_INTEL_IPU4_DS90UB954_MOPAR
					 len & CRL_REG_LEN_READ_MASK_MOPAR, &val2);
#else
...
#ifdef CONFIG_INTEL_IPU4_DS90UB954_MOPAR
				  len & CRL_REG_LEN_READ_MASK_MOPAR, val);
#else
	...
	if ((sensor->sensor_ds->i2c_mutex_in_use)
#ifdef CONFIG_INTEL_IPU4_DS90UB954_MOPAR
			&& (len & CRL_REG_READ_AND_UPDATE_MOPAR))
#else
