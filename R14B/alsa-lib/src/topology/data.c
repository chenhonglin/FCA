...
static int tplg_parse_data_hex(snd_config_t *cfg, struct tplg_elem *elem,
	int width)
{
	struct snd_soc_tplg_private *priv, *new_priv;
	const char *value = NULL;
...
{
	struct snd_soc_tplg_private *priv = elem->data, *new_priv;
	...
	for (i = 0; i < tuples->num_sets ; i++) {
		...
		if (priv != NULL) {
			new_priv = realloc(priv, sizeof(*priv) + size);
			if(!new_priv) {
				free(priv);
				return -ENOMEM;
			} else {
