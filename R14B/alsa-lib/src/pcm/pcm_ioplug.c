...
static void snd_pcm_ioplug_hw_ptr_update(snd_pcm_t *pcm)
{
	...
	if (hw >= 0) {
		...
		if (delta == 0) {
			if (io->data->callback->get_real_avail) {
				if ((io->data->callback->get_real_avail(io->data) == (snd_pcm_sframes_t)pcm->buffer_size)
						&& (snd_pcm_mmap_avail(pcm) == 0)) {
                                    /* the buffer maintained by the plugin has buffer_size frames available,
                                     * however the ioplug missed some updates of the hw_ptr due to scheduling issues
                                     * and thus still has the information that there are no frames available.
                                     * To fix that gap we have to forward the hw_ptr by buffer_size. */
					delta = pcm->buffer_size;
				}
			}
		}
